...
import type { FilingEvidenceRating, FilingEvidenceGovernanceResult } from "@/lib/engines/home-filing-evidence-governance-intelligence-engine";

function useHomeFilingEvidenceGovernanceIntelligence() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: FilingEvidenceGovernanceResult }>;
    },
      ...
      <CardContent className="space-y-4">
        {d.filing_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.verified_rate === null ? "bg-slate-50" : d.verified_rate >= 90 ? "bg-green-50" : d.verified_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.verified_rate === null ? "text-muted-foreground" : d.verified_rate >= 90 ? "text-[--cs-success]" : d.verified_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.verified_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Verified</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.description_rate === null ? "bg-slate-50" : d.description_rate >= 95 ? "bg-green-50" : d.description_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.description_rate === null ? "text-muted-foreground" : d.description_rate >= 95 ? "text-[--cs-success]" : d.description_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.description_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Described</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.significant_event_filing_rate === null ? "bg-slate-50" : d.significant_event_filing_rate >= 100 ? "bg-green-50" : d.significant_event_filing_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.significant_event_filing_rate === null ? "text-muted-foreground" : d.significant_event_filing_rate >= 100 ? "text-[--cs-success]" : d.significant_event_filing_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.significant_event_filing_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Sig Filed</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.linked_rate === null ? "bg-slate-50" : d.linked_rate >= 90 ? "bg-green-50" : d.linked_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.linked_rate === null ? "text-muted-foreground" : d.linked_rate >= 90 ? "text-[--cs-success]" : d.linked_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.linked_rate)}</p>
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Filing Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
