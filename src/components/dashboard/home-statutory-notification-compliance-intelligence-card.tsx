...
import type { StatutoryNotificationRating, StatutoryNotificationComplianceResult } from "@/lib/engines/home-statutory-notification-compliance-intelligence-engine";

function useHomeStatutoryNotificationComplianceIntelligence() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: StatutoryNotificationComplianceResult }>;
    },
      ...
      <CardContent className="space-y-4">
        {d.notification_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.timeliness_rate === null ? "bg-slate-50" : d.timeliness_rate >= 100 ? "bg-green-50" : d.timeliness_rate >= 90 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.timeliness_rate === null ? "text-muted-foreground" : d.timeliness_rate >= 100 ? "text-[--cs-success]" : d.timeliness_rate >= 90 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.timeliness_rate)}</p>
              <p className="text-[9px] text-muted-foreground">On Time</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.completeness_rate === null ? "bg-slate-50" : d.completeness_rate >= 100 ? "bg-green-50" : d.completeness_rate >= 90 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.completeness_rate === null ? "text-muted-foreground" : d.completeness_rate >= 100 ? "text-[--cs-success]" : d.completeness_rate >= 90 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.completeness_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Complete</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.follow_up_rate === null ? "bg-slate-50" : d.follow_up_rate >= 100 ? "bg-green-50" : d.follow_up_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.follow_up_rate === null ? "text-muted-foreground" : d.follow_up_rate >= 100 ? "text-[--cs-success]" : d.follow_up_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.follow_up_rate)}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.acknowledgement_rate === null ? "bg-slate-50" : d.acknowledgement_rate >= 90 ? "bg-green-50" : d.acknowledgement_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.acknowledgement_rate === null ? "text-muted-foreground" : d.acknowledgement_rate >= 90 ? "text-[--cs-success]" : d.acknowledgement_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.acknowledgement_rate)}</p>
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Notification Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
