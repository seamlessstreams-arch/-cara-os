...
describe("consent-capacity-monitoring-service", () => {
  describe("computeConsentCapacityMetrics", () => {
    it("returns zeros for empty", () => { 
      ...
      expect(m.best_interest_count).toBe(0);
      expect(m.child_views_rate).toBeNull();;
      expect(m.unique_children).toBe(0);
    });
    it("returns empty breakdowns", () => { 
      const m = computeConsentCapacityMetrics([]);
      expect(m.by_consent_area).toEqual({});
      expect(m.by_capacity_level).toEqual({});
      expect(m.by_decision_type).toEqual({});
      expect(m.by_competence_assessment).toEqual({});
    });
    ...
    it("returns 100% boolean rates with defaults", () => { 
      const m = computeConsentCapacityMetrics([makeRecord()]);
      expect(m.child_views_rate).toBe(100);
      expect(m.information_provided_rate).toBe(100);
      expect(m.age_appropriate_rate).toBe(100);
      expect(m.advocacy_rate).toBe(100);
      expect(m.parent_consulted_rate).toBe(100);
      expect(m.social_worker_rate).toBe(100);
      expect(m.best_interest_documented_rate).toBe(100);
      expect(m.decision_respected_rate).toBe(100);
      expect(m.review_date_rate).toBe(100);
      expect(m.care_plan_rate).toBe(100);
      expect(m.legal_framework_rate).toBe(100);
  ...
  describe("identifyConsentCapacityAlerts", () => {
    ...
    it("fires best_interest_not_documented", () => { 
      const a = identifyConsentCapacityAlerts([makeRecord({ decision_type: "best_interest_decision", best_interest_documented: false, child_name: "Jo" })]);
      expect(a[0].type).toBe("best_interest_not_documented");
    ...
    it("fires decision_not_respected singular", () => { 
      const a = identifyConsentCapacityAlerts([makeRecord({ decision_respected: false })]);
      const f = a.find(x => x.type === "decision_not_respected");
    ...
    it("fires advocacy_not_offered singular", () => { 
      const a = identifyConsentCapacityAlerts([makeRecord({ advocacy_offered: false })]);
      const f = a.find(x => x.type === "advocacy_not_offered");
      expect(f).toBeDefined();
      expect(f!.severity).toBe("high");
