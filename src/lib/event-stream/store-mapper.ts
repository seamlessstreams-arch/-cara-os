...
export function mapStoreToEventInput(store: any): EventProjectorInput {
  return {
    ...
    complaints: ((store.complaints ?? []) as any[]).map((c) => ({
    ...
    familyContacts: ((intelligenceDb.contactLogs.findAll("home_oak") ?? []) as any[]).map((c) => ({
    ...
    riskAssessments: ((store.riskAssessments ?? []) as any[]).map((r) => ({
    ...
    lacReviews: ((store.lacReviews ?? []) as any[]).map((r) => ({
    ...
    notifiableEvents: ((store.notifiableEvents ?? []) as any[]).map((r) => ({
    ...
    behaviourSupportPlans: ((store.behaviourSupportPlans ?? []) as any[]).map((r) => ({
      ...
      primary_behaviours: ((r.primary_behaviours ?? []) as any[]).map((b) => ({ severity: b.severity })),
