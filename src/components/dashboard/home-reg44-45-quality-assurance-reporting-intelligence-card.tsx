...
import type { Reg4445QualityRating, Reg4445QualityAssuranceReportingResult } from "@/lib/engines/home-reg44-45-quality-assurance-reporting-intelligence-engine";
...
function useHomeReg4445QualityAssuranceReportingIntelligence() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: Reg4445QualityAssuranceReportingResult }>;
    },
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-violet-600 border-2")}>
      ...
      <CardContent className="space-y-4">
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Reg 44/45 Quality Assurance Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
