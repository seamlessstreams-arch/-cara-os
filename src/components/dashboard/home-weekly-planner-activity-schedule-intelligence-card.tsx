...
import type { WeeklyPlannerRating, WeeklyPlannerResult } from "@/lib/engines/home-weekly-planner-activity-schedule-intelligence-engine";
...
export function HomeWeeklyPlannerActivityScheduleIntelligenceCard() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: WeeklyPlannerResult }>;
    },
      ...
      <CardContent className="space-y-4">
        {d.planner_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", meets(d.schedule_timeliness_rate, 90) ? "bg-green-50" : meets(d.schedule_timeliness_rate, 70) ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", meets(d.schedule_timeliness_rate, 90) ? "text-[--cs-success]" : meets(d.schedule_timeliness_rate, 70) ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.schedule_timeliness_rate)}</p>
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Weekly Planner & Activity Schedule Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
