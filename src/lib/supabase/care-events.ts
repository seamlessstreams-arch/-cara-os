...
export const sbCareEventJobs = {
  ...
  async upsert(data: Omit<CareEventJob, "id" | "created_at" | "updated_at">): Promise<CareEventJob> {
    ...
    if (existing && existing.length > 0) {
      const { data: up, error } = await sb
        .from("care_event_jobs").update(careEventJobToRow(data)).eq("id", existing[0].id).select().single();
      if (error) throw error;
      return rowToCareEventJob(up);
    }
    const { data: ins, error } = await sb
      .from("care_event_jobs").insert(careEventJobToRow(data)).select().single();
    if (error) throw error;
  ...
  async patch(id: string, data: Partial<CareEventJob>): Promise<CareEventJob | null> {
    const sb = supabase();
    const { data: up, error } = await sb
      .from("care_event_jobs").update(careEventJobToRow(data)).eq("id", id).select().single();
    if (error) throw error;
