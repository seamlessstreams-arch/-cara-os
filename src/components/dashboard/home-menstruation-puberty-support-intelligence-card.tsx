...
import type { MenstruationPubertyRating, MenstruationPubertyResult } from "@/lib/engines/home-menstruation-puberty-support-intelligence-engine";
...
export function HomeMenstruationPubertySupportIntelligenceCard() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: MenstruationPubertyResult }>;
    },
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-purple-600 border-2")}>
      ...
      <CardContent className="space-y-4">
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Menstruation & Puberty Support Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
