    ...
    <Card className="overflow-hidden border-slate-200">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-4 gap-2">
          <div className={cn("text-center rounded-lg p-2", att.band === "insufficient_data" ? "bg-slate-50" : (att.overall_pct ?? 0) >= 96 ? "bg-green-50" : (att.overall_pct ?? 0) >= 90 ? "bg-blue-50" : (att.overall_pct ?? 0) >= 85 ? "bg-amber-50" : "bg-red-50")}>
            <p className={cn("text-lg font-bold tabular-nums", bandStyle)}>{att.band === "insufficient_data" ? "N/A" : `${formatRate(att.overall_pct)}`}</p>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <div className="rounded border p-2 flex items-center gap-2 text-xs">
            ...
            <div className="min-w-0">
              ...
              {pep.targets_set > 0 && (
                <p className="text-[10px] text-muted-foreground">
                  Targets: {pep.targets_achieved}/{pep.targets_set} achieved ({formatRate(pep.target_achievement_rate)})
                </p>
              )}
            </div>
          </div>
            ...
            {d.homework.total_sessions_30d > 0 && (
              <div className="rounded border p-2 flex items-center gap-2 text-xs">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {d.homework.total_sessions_30d} sessions · {formatRate(d.homework.completion_rate)} complete · {formatRate(d.homework.engagement_rate)} engaged
                  </p>
                </div>
              </div>
            )}
