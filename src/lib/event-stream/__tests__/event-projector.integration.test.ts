...
describe("event-projector integration (real seed data)", () => {
  ...
  const result = buildEventStream({
    ...
    appointments: ((store.appointments)).map((a) => ({
      id: a.id, child_id: a.child_id, date: d(a.date ?? a.created_at), time: a.time, type: a.type, title: a.title, status: a.status, outcome: a.outcome, recorded_by: a.recorded_by, created_at: a.created_at,
    })),
