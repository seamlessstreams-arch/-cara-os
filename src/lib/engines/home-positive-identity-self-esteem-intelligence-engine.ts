...
export interface PositiveIdentityResult {
  ...
  identity_work_rate: number | null;
...
export function computePositiveIdentitySelfEsteem(
  ...
  const identityWorkRate = rate(completedIdentityWork, totalIdentityRecords);

  const identityChildEngaged = identity_work_records.filter((r) => r.child_engaged).length;
  const identityEngagementRate = rate(identityChildEngaged, totalIdentityRecords);

  const identityChildLed = identity_work_records.filter((r) => r.child_led).length;
  const identityChildLedRate = rate(identityChildLed, totalIdentityRecords);

  const identityTherapeutic = identity_work_records.filter((r) => r.therapeutic_support).length;
  const identityTherapeuticRate = rate(identityTherapeutic, totalIdentityRecords);

  const identityOutcomesDocumented = identity_work_records.filter((r) => r.outcomes_documented).length;
  const identityOutcomesRate = rate(identityOutcomesDocumented, totalIdentityRecords);
  ...
  const lifeStoryBookRate = rate(withLifeStoryBook, totalLifeStoryRecords);

  const lifeStoryActive = life_story_records.filter((r) => r.life_story_work_active).length;
  const lifeStoryActiveRate = rate(lifeStoryActive, totalLifeStoryRecords);
  ...
  const lifeStorySessionRate = rate(completedLifeStorySessions, totalLifeStorySessions);

  const lifeStoryChildEngaged = life_story_records.filter((r) => r.child_engaged).length;
  const lifeStoryEngagementRateRaw = rate(lifeStoryChildEngaged, totalLifeStoryRecords);

  const lifeStoryStaffTrained = life_story_records.filter((r) => r.staff_trained).length;
  const lifeStoryStaffTrainingRate = rate(lifeStoryStaffTrained, totalLifeStoryRecords);
  ...
  const lifeStoryEngagementRate: number | null =
    totalLifeStoryRecords > 0
      ? meanOf([lifeStoryBookRate, lifeStoryActiveRate, lifeStorySessionRate, lifeStoryEngagementRateRaw])
      : null;
  ...
  const selfEsteemAttendanceRate = rate(attendedSelfEsteemSessions, totalSelfEsteemSessions);

  const selfEsteemEngaged = self_esteem_programme_records.filter((r) => r.child_engaged).length;
  const selfEsteemEngagementRate = rate(selfEsteemEngaged, totalSelfEsteemRecords);

  const selfEsteemProgressDocumented = self_esteem_programme_records.filter((r) => r.progress_documented).length;
  const selfEsteemProgressRate = rate(selfEsteemProgressDocumented, totalSelfEsteemRecords);

  const selfEsteemMeasurable = self_esteem_programme_records.filter((r) => r.measurable_outcomes).length;
  const selfEsteemMeasurableRate = rate(selfEsteemMeasurable, totalSelfEsteemRecords);

  const selfEsteemEvidenceBased = self_esteem_programme_records.filter((r) => r.evidence_based).length;
  const selfEsteemEvidenceBasedRate = rate(selfEsteemEvidenceBased, totalSelfEsteemRecords);
  ...
  const selfEsteemProgrammeRate: number | null =
    totalSelfEsteemRecords > 0
      ? meanOf([selfEsteemAttendanceRate, selfEsteemEngagementRate, selfEsteemProgressRate])
      : null;
  ...
  const achievementCelebrationRate = rate(celebratedAchievements, totalAchievementRecords);

  const displayedAchievements = achievement_records.filter((r) => r.displayed).length;
  const achievementDisplayRate = rate(displayedAchievements, totalAchievementRecords);

  const sharedWithFamily = achievement_records.filter((r) => r.shared_with_family).length;
  const achievementFamilyShareRate = rate(sharedWithFamily, totalAchievementRecords);

  const childProud = achievement_records.filter((r) => r.child_proud).length;
  const childPrideRate = rate(childProud, totalAchievementRecords);

  const peersAcknowledged = achievement_records.filter((r) => r.peers_acknowledged).length;
  const peerAcknowledgementRate = rate(peersAcknowledged, totalAchievementRecords);

  const recordedInCarePlan = achievement_records.filter((r) => r.recorded_in_care_plan).length;
  const achievementCarePlanRate = rate(recordedInCarePlan, totalAchievementRecords);
  ...
  const positiveImageCompletionRate = rate(completedPositiveImage, totalPositiveImageRecords);

  const positiveImageEngaged = positive_image_records.filter((r) => r.child_engaged).length;
  const positiveImageEngagementRate = rate(positiveImageEngaged, totalPositiveImageRecords);

  const positiveImageChildLed = positive_image_records.filter((r) => r.child_led).length;
  const positiveImageChildLedRate = rate(positiveImageChildLed, totalPositiveImageRecords);

  const positiveImageImprovement = positive_image_records.filter((r) => r.measurable_improvement).length;
  const positiveImageImprovementRate = rate(positiveImageImprovement, totalPositiveImageRecords);

  // Composite positive image rate
  // fab-0: null when no records.
  const positiveImageRate: number | null =
    totalPositiveImageRecords > 0 ? Math.round((positiveImageCompletionRate! + positiveImageEngagementRate! + positiveImageImprovementRate!) / 3) : null;
  ...
  const childConfidenceRate = rate(confidenceNumerator, confidenceDenominator);
  ...
  if (meets(identityWorkRate, 90)) score += 4;
  else if (meets(identityWorkRate, 70)) score += 2;
  ...
  if (meets(achievementDisplayRate, 90)) score += 3;
  else if (meets(achievementDisplayRate, 70)) score += 1;

  // --- Bonus 8: lifeStoryBookRate (>=90: +3, >=70: +1) ---
  if (meets(lifeStoryBookRate, 90)) score += 3;
  else if (meets(lifeStoryBookRate, 70)) score += 1;

  // --- Bonus 9: selfEsteemEvidenceBasedRate (>=80: +2, >=50: +1) ---
  if (meets(selfEsteemEvidenceBasedRate, 80)) score += 2;
  else if (meets(selfEsteemEvidenceBasedRate, 50)) score += 1;

  // -- Penalties (4 with guards) -------------------------------------------

  // identityWorkRate < 50 -> -5
  if (below(identityWorkRate, 50) && totalIdentityRecords > 0) score -= 5;
  ...
  if (below(achievementCelebrationRate, 50) && totalAchievementRecords > 0) score -= 4;
  ...
  if (meets(identityWorkRate, 90) && totalIdentityRecords > 0) {
    strengths.push(
  ...
  } else if (meets(identityWorkRate, 70) && totalIdentityRecords > 0) {
    strengths.push(
  ...
  if (meets(identityEngagementRate, 80) && totalIdentityRecords > 0) {
    strengths.push(
  ...
  if (meets(identityChildLedRate, 50) && totalIdentityRecords > 0) {
    strengths.push(
  ...
  if (meets(identityTherapeuticRate, 50) && totalIdentityRecords > 0) {
    strengths.push(
  ...
  if (meets(lifeStoryBookRate, 90) && totalLifeStoryRecords > 0) {
    strengths.push(
  ...
  } else if (meets(lifeStoryBookRate, 70) && totalLifeStoryRecords > 0) {
    strengths.push(
  ...
  if (meets(lifeStorySessionRate, 80) && totalLifeStorySessions > 0) {
    strengths.push(
  ...
  if (meets(lifeStoryStaffTrainingRate, 80) && totalLifeStoryRecords > 0) {
    strengths.push(
  ...
  if (meets(selfEsteemEvidenceBasedRate, 80) && totalSelfEsteemRecords > 0) {
    strengths.push(
  ...
  if (meets(selfEsteemMeasurableRate, 70) && totalSelfEsteemRecords > 0) {
    strengths.push(
  ...
  if (meets(achievementCelebrationRate, 90) && totalAchievementRecords > 0) {
    strengths.push(
  ...
  } else if (meets(achievementCelebrationRate, 70) && totalAchievementRecords > 0) {
    strengths.push(
  ...
  if (meets(achievementDisplayRate, 80) && totalAchievementRecords > 0) {
    strengths.push(
  ...
  if (meets(achievementFamilyShareRate, 70) && totalAchievementRecords > 0) {
    strengths.push(
  ...
  if (meets(childPrideRate, 80) && totalAchievementRecords > 0) {
    strengths.push(
  ...
  if (meets(peerAcknowledgementRate, 70) && totalAchievementRecords > 0) {
    strengths.push(
  ...
  if (meets(positiveImageImprovementRate, 70) && totalPositiveImageRecords > 0) {
    strengths.push(
  ...
  if (meets(positiveImageChildLedRate, 50) && totalPositiveImageRecords > 0) {
    strengths.push(
  ...
  if (meets(achievementCarePlanRate, 80) && totalAchievementRecords > 0) {
    strengths.push(
  ...
  if (below(identityWorkRate, 50) && totalIdentityRecords > 0) {
    concerns.push(
  ...
  } else if (below(identityWorkRate, 70) && meets(identityWorkRate, 50) && totalIdentityRecords > 0) {
    concerns.push(
  ...
  if (below(identityEngagementRate, 50) && totalIdentityRecords > 0) {
    concerns.push(
  ...
  if (below(identityOutcomesRate, 50) && totalIdentityRecords > 0) {
    concerns.push(
  ...
  if (below(lifeStoryBookRate, 50) && totalLifeStoryRecords > 0) {
    concerns.push(
  ...
  } else if (below(lifeStoryBookRate, 70) && meets(lifeStoryBookRate, 50) && totalLifeStoryRecords > 0) {
    concerns.push(
  ...
  if (below(lifeStorySessionRate, 50) && totalLifeStorySessions > 0) {
    concerns.push(
  ...
  if (below(lifeStoryStaffTrainingRate, 50) && totalLifeStoryRecords > 0) {
    concerns.push(
  ...
  if (below(selfEsteemAttendanceRate, 50) && totalSelfEsteemSessions > 0) {
    concerns.push(
  ...
  if (below(selfEsteemMeasurableRate, 40) && totalSelfEsteemRecords > 0) {
    concerns.push(
  ...
  if (below(selfEsteemEvidenceBasedRate, 30) && totalSelfEsteemRecords > 0) {
    concerns.push(
  ...
  if (below(achievementCelebrationRate, 50) && totalAchievementRecords > 0) {
    concerns.push(
  ...
  } else if (below(achievementCelebrationRate, 70) && meets(achievementCelebrationRate, 50) && totalAchievementRecords > 0) {
    concerns.push(
      `Achievement celebration at ${achievementCelebrationRate}% -- not all children's successes are being recognised and celebrated.`,
    );
  }

  if (below(achievementDisplayRate, 50) && totalAchievementRecords > 0) {
    concerns.push(
  ...
  if (below(childPrideRate, 50) && totalAchievementRecords > 0) {
    concerns.push(
  ...
  if (below(achievementCarePlanRate, 50) && totalAchievementRecords > 0) {
    concerns.push(
  ...
  if (below(positiveImageImprovementRate, 40) && totalPositiveImageRecords > 0) {
    concerns.push(
  ...
  if (below(achievementFamilyShareRate, 50) && totalAchievementRecords > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (meets(identityWorkRate, 50) && below(identityWorkRate, 70) && totalIdentityRecords > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (meets(achievementCelebrationRate, 50) && below(achievementCelebrationRate, 70) && totalAchievementRecords > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (below(childConfidenceRate, 40) && confidenceDenominator > 0) {
    insights.push({
  ...
  if (meets(childConfidenceRate, 50) && below(childConfidenceRate, 60) && confidenceDenominator > 0) {
    insights.push({
  ...
  if (meets(lifeStoryBookRate, 50) && below(lifeStoryBookRate, 70) && totalLifeStoryRecords > 0) {
    insights.push({
  ...
  if (meets(selfEsteemEvidenceBasedRate, 30) && below(selfEsteemEvidenceBasedRate, 80) && totalSelfEsteemRecords > 0) {
    insights.push({
  ...
  if (meets(achievementCarePlanRate, 50) && below(achievementCarePlanRate, 80) && totalAchievementRecords > 0) {
    insights.push({
  ...
  if (totalIdentityRecords > 0 && totalLifeStoryRecords > 0 && meets(lifeStoryEngagementRate, 80) && meets(identityWorkRate, 90)) {
    insights.push({
  ...
  if (meets(achievementCelebrationRate, 90) && meets(childPrideRate, 80) && totalAchievementRecords > 0) {
    insights.push({
  ...
  if (totalSelfEsteemRecords > 0 && meets(selfEsteemProgrammeRate, 80) && meets(selfEsteemMeasurableRate, 70)) {
    insights.push({
  ...
  if (totalPositiveImageRecords > 0 && meets(positiveImageRate, 80) && meets(positiveImageImprovementRate, 70)) {
    insights.push({
  ...
  if (meets(achievementDisplayRate, 80) && meets(peerAcknowledgementRate, 70) && totalAchievementRecords > 0) {
    insights.push({
  ...
  if (totalLifeStoryRecords > 0 && lifeStorySatisfactionAvg !== null && meets(lifeStoryBookRate, 90) && lifeStorySatisfactionAvg >= 4.0) {
    insights.push({
