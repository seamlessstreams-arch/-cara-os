...
import type { DeprivationOfLibertyRating, DeprivationOfLibertyResult } from "@/lib/engines/home-deprivation-of-liberty-intelligence-engine";
...
export function HomeDeprivationOfLibertyIntelligenceCard() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: DeprivationOfLibertyResult }>;
    },
      ...
      <CardContent className="space-y-4">
        {d.dol_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_consultation_rate === null ? "bg-slate-50" : d.child_consultation_rate >= 95 ? "bg-green-50" : d.child_consultation_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_consultation_rate === null ? "text-muted-foreground" : d.child_consultation_rate >= 95 ? "text-[--cs-success]" : d.child_consultation_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.child_consultation_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Consulted</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.sw_consultation_rate === null ? "bg-slate-50" : d.sw_consultation_rate >= 95 ? "bg-green-50" : d.sw_consultation_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.sw_consultation_rate === null ? "text-muted-foreground" : d.sw_consultation_rate >= 95 ? "text-[--cs-success]" : d.sw_consultation_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.sw_consultation_rate)}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.impact_assessment_rate === null ? "bg-slate-50" : d.impact_assessment_rate >= 90 ? "bg-green-50" : d.impact_assessment_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.impact_assessment_rate === null ? "text-muted-foreground" : d.impact_assessment_rate >= 90 ? "text-[--cs-success]" : d.impact_assessment_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.impact_assessment_rate)}</p>
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Liberty & Rights Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
