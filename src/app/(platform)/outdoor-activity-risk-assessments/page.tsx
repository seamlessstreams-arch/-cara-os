      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            <div><Label htmlFor="284c-activity-name">Activity Name *</Label><Input id="284c-activity-name" placeholder="e.g. Trip to Beamish Museum" value={form.activity_name} onChange={(e) => setForm((f) => ({ ...f, activity_name: e.target.value }))} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label htmlFor="284c-activity-type">Activity Type</Label><Select value={form.activity_type} onValueChange={(v) => setForm((f) => ({ ...f, activity_type: v as OutdoorActivityType }))}><SelectTrigger id="284c-activity-type"><SelectValue /></SelectTrigger><SelectContent>{(Object.entries(OUTDOOR_ACTIVITY_TYPE_LABEL) as [OutdoorActivityType, string][]).map(([v, l]) => <SelectItem key={v} value={v}>{l}</SelectItem>)}</SelectContent></Select></div>
              <div><Label htmlFor="284c-date">Date *</Label><Input id="284c-date" type="date" value={form.date} onChange={(e) => setForm((f) => ({ ...f, date: e.target.value }))} /></div>
            </div>
            <div><Label htmlFor="284c-location">Location *</Label><Input id="284c-location" placeholder="e.g. Beamish, County Durham" value={form.location} onChange={(e) => setForm((f) => ({ ...f, location: e.target.value }))} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label htmlFor="284c-duration-hours">Duration (hours)</Label><Input id="284c-duration-hours" type="number" min={0.5} max={24} step={0.5} value={form.duration_hours} onChange={(e) => setForm((f) => ({ ...f, duration_hours: parseFloat(e.target.value) }))} /></div>
              <div><Label htmlFor="284c-lead-staff">Lead Staff *</Label><Select value={form.lead_staff} onValueChange={(v) => setForm((f) => ({ ...f, lead_staff: v }))}><SelectTrigger id="284c-lead-staff"><SelectValue placeholder="Select staff" /></SelectTrigger><SelectContent>{STAFF.map((s) => <SelectItem key={s.id} value={s.id}>{s.full_name}</SelectItem>)}</SelectContent></Select></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label htmlFor="284c-behaviour-risk">Behaviour Risk</Label><Select value={form.behaviour_risk_rating} onValueChange={(v) => setForm((f) => ({ ...f, behaviour_risk_rating: v as OutdoorRiskLevel }))}><SelectTrigger id="284c-behaviour-risk"><SelectValue /></SelectTrigger><SelectContent>{(Object.entries(OUTDOOR_RISK_LEVEL_LABEL) as [OutdoorRiskLevel, string][]).map(([v, l]) => <SelectItem key={v} value={v}>{l}</SelectItem>)}</SelectContent></Select></div>
              <div><Label htmlFor="284c-missing-risk">Missing Risk</Label><Select value={form.missing_from_care_risk} onValueChange={(v) => setForm((f) => ({ ...f, missing_from_care_risk: v as OutdoorRiskLevel }))}><SelectTrigger id="284c-missing-risk"><SelectValue /></SelectTrigger><SelectContent>{(Object.entries(OUTDOOR_RISK_LEVEL_LABEL) as [OutdoorRiskLevel, string][]).map(([v, l]) => <SelectItem key={v} value={v}>{l}</SelectItem>)}</SelectContent></Select></div>
            </div>
          </div>
          <DialogFooter>
