...
export function computeOutcomeStarNeeds(
  ...
  const assessmentCoverage = rate(childrenAssessed, total_children);
  ...
  const improvementRate = rate(childrenImproving, starsWithPrevious.length);

  // Child participation
  const participatingStars = outcome_stars.filter(s => s.child_participated).length;
  const participationRate = rate(participatingStars, outcome_stars.length);
  ...
  const kpiMetRate = rate(kpisMet, kpis.length);
  ...
  if (meets(assessmentCoverage, 90)) {
    strengths.push(`${formatRate(assessmentCoverage)} of children have Outcome Star assessments — comprehensive assessment coverage across the home.`);
  }
  ...
  if (starsWithPrevious.length > 0 && meets(improvementRate, 70)) {
    strengths.push(`${formatRate(improvementRate)} of children with previous assessments are improving — clear upward trajectory in outcomes.`);
  }
  if (meets(participationRate, 90) && outcome_stars.length > 0) {
    strengths.push(`${formatRate(participationRate)} child participation in Outcome Star assessments — excellent child engagement in their own progress review.`);
  }
  if (meets(needsAddressedRate, 85) && needs_assessments.length > 0) {
    strengths.push(`${formatRate(needsAddressedRate)} of identified needs are being addressed — responsive and effective needs management.`);
  }
  if (meets(kpiMetRate, 80) && kpis.length > 0) {
    strengths.push(`${formatRate(kpiMetRate)} of KPIs met — the home is consistently achieving its outcome targets.`);
  }
  if (outcome_stars.length > 0) {
    const withActionPlan = outcome_stars.filter(s => s.has_action_plan).length;
    const actionPlanRate = rate(withActionPlan, outcome_stars.length);
    if (meets(actionPlanRate, 90)) {
      strengths.push(`${formatRate(actionPlanRate)} of Outcome Star assessments have associated action plans — strong planning discipline.`);
    }
  }

  // ── Concerns ──────────────────────────────────────────────────────
  if (below(assessmentCoverage, 50)) {
    concerns.push(`Only ${formatRate(assessmentCoverage)} of children have Outcome Star assessments — majority of children's outcomes are not being tracked.`);
  }
  ...
  if (starsWithPrevious.length > 0 && below(improvementRate, 30)) {
    concerns.push(`Only ${formatRate(improvementRate)} of children are improving — the majority are stagnating or declining in their outcomes.`);
  }
  if (below(participationRate, 50) && outcome_stars.length > 0) {
    concerns.push(`Only ${formatRate(participationRate)} child participation in assessments — children's voices are not being heard in their own progress reviews.`);
  }
  if (below(needsAddressedRate, 50) && needs_assessments.length > 0) {
    concerns.push(`Only ${formatRate(needsAddressedRate)} of identified needs are being addressed — significant unmet needs across the home.`);
  }
  if (below(kpiMetRate, 40) && kpis.length > 0) {
    concerns.push(`Only ${formatRate(kpiMetRate)} of KPIs met — the home is failing to meet the majority of its outcome targets.`);
  }
  ...
  // ── Recommendations ───────────────────────────────────────────────
  if (below(assessmentCoverage, 50) && total_children > 0) {
  ...
  if (rating === "outstanding") {
    insights.push({ text: `Exemplary outcomes practice — ${childrenAssessed} children assessed with an average score of ${avgOutcomeScore}/10, ${formatRate(improvementRate)} improving, and ${formatRate(needsAddressedRate)} of needs addressed. Ofsted inspectors will find robust evidence that the home is making a measurable, sustained difference to children's lives.`, severity: "positive" });
  }
  if (rating === "good") {
    insights.push({ text: `Good outcomes framework in place — ${childrenAssessed} children assessed with ${formatRate(improvementRate)} showing improvement. The home demonstrates consistent progress monitoring, though there may be opportunities to strengthen coverage or child participation further.`, severity: "positive" });
  }
  if (below(assessmentCoverage, 50) && total_children > 0) {
    insights.push({ text: `Only ${formatRate(assessmentCoverage)} of children have Outcome Star assessments. Regulation 5 requires the registered person to ensure care is provided in a way that promotes each child's welfare. Without systematic outcome measurement, the home cannot demonstrate it is meeting this standard.`, severity: "critical" });
  }
  if (starsWithPrevious.length > 0 && below(improvementRate, 30)) {
    insights.push({ text: `Only ${formatRate(improvementRate)} of children are improving between assessments. Under SCCIF, inspectors specifically look for evidence that children are making progress. A low improvement rate across the home may indicate that care plans and interventions need fundamental review.`, severity: "critical" });
  }
  if (below(needsAddressedRate, 50) && needs_assessments.length > 0) {
    insights.push({ text: `Only ${formatRate(needsAddressedRate)} of identified needs are being addressed. Unmet needs represent a safeguarding risk and demonstrate a gap between assessment and delivery. Inspectors will expect to see that identified needs result in action.`, severity: "critical" });
  }
  if (meets(participationRate, 90) && outcome_stars.length > 0 && starsWithPrevious.length > 0 && meets(improvementRate, 70)) {
    insights.push({ text: `Strong correlation between high child participation (${formatRate(participationRate)}) and strong improvement rates (${formatRate(improvementRate)}) — children who are engaged in their own assessments are more likely to make progress. This evidences genuinely child-centred practice.`, severity: "positive" });
  }
  if (meets(kpiMetRate, 80) && kpis.length > 0 && meets(needsAddressedRate, 85) && needs_assessments.length > 0) {
    insights.push({ text: `KPI achievement of ${formatRate(kpiMetRate)} aligns with ${formatRate(needsAddressedRate)} needs addressed rate — the home's performance targets are translating into real improvements for children.`, severity: "positive" });
  }

  // ── Headline ──────────────────────────────────────────────────────
  let headline: string;
  if (rating === "outstanding") {
    headline = `Outstanding outcomes practice — ${childrenAssessed} children assessed, average score ${avgOutcomeScore}/10, ${formatRate(improvementRate)} improving with ${formatRate(needsAddressedRate)} of needs addressed.`;
  } else if (rating === "good") {
    headline = `Good outcomes tracking — ${childrenAssessed} children assessed with ${formatRate(improvementRate)} showing improvement across Outcome Star domains.`;
  } else if (rating === "adequate") {
    headline = `Adequate outcomes framework — progress is mixed with ${below(assessmentCoverage, 50) ? "limited assessment coverage" : "inconsistent improvement trajectories"} requiring attention.`;
  } else {
    headline = `Inadequate outcomes practice — ${below(assessmentCoverage, 50) ? "significant assessment gaps" : "poor improvement rates and unmet needs"} require urgent action.`;
  }
