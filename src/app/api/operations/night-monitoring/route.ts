...
export async function POST(request: NextRequest) {
  try {
    ...
    const __fd = rejectFutureDates(body, ["shiftDate"]); if (__fd) return __fd;
    ...
    const CLAIM_FIELDS: Record<string, readonly string[]> = {
      create_log: ["premisesSecure", "firePanelChecked"],
    };
    ...
    if (action === "create_log") {
      const result = await createNightLog({
        ...
        premisesSecure: body.premisesSecure,
        firePanelChecked: body.firePanelChecked,
