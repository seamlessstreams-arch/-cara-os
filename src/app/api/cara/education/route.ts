...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: "Education intelligence failed" },
      { status: 500 },
    );
  }
}
...
async function fetchData(sb: NonNullable<ReturnType<typeof createServerClient>>, childId: string): Promise<EducationInput> {
  ...
  return {
    ...
    inEducation: config?.in_education ?? null,
    attendanceRecords,
    attendanceTrend: config?.attendance_trend ?? "stable",
    exclusions,
    pepRecords,
    pepDue: config?.pep_due ?? false,
    onTrackForTargets: config?.on_track ?? null,
    progressRating: config?.progress_rating ?? "expected",
    sendSupport: config?.send_support ?? false,
    ehcpInPlace: config?.ehcp ?? false,
    designatedTeacherEngaged: config?.dt_engaged ?? null,
    virtualSchoolInvolved: config?.vs_involved ?? null,
    tutoring: config?.tutoring ?? false,
    mentoring: config?.mentoring ?? false,
    ppPlusEffectivelyUsed: config?.pp_effective ?? null,
    childEnjoysSChool: config?.enjoys_school ?? null,
    homeworkSupported: config?.homework_supported ?? null,
    aspirationsDiscussed: config?.aspirations ?? null,
