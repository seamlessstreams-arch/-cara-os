...
export interface ClothingPossessionsResult {
  ...
  allowance_utilisation_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  wardrobe_adequacy_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  inventory_completeness_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  request_fulfilment_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  possession_safeguarding_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
...
function emptyResult(
  ...
  return {
    ...
    allowance_utilisation_rate: null,
    wardrobe_adequacy_rate: null,
    inventory_completeness_rate: null,
    request_fulfilment_rate: null,
    possession_safeguarding_rate: null,
...
export function computeClothingPersonalPossessions(
  ...
  const allowanceUtilisationRate = rate(totalSpent, totalAllowance);

  const childInvolvedShopping = clothing_allowance_records.filter(
    (r) => r.child_involved_in_shopping,
  ).length;
  const childInvolvedShoppingRate = rate(childInvolvedShopping, totalAllowanceRecords);
  ...
  const receiptsRetainedRate = rate(receiptsRetained, totalAllowanceRecords);
  ...
  const allowanceChildCoverage =
    total_children > 0 ? rate(uniqueChildrenWithAllowance, total_children) : 0;
  ...
  const wardrobeAdequacyRate = rate(adequateWardrobes, totalWardrobeReviews);
  ...
  const childSatisfiedRate = rate(childSatisfied, totalWardrobeReviews);

  const culturalReligiousMet = wardrobe_review_records.filter(
    (r) => r.cultural_religious_needs_met,
  ).length;
  const culturalReligiousRate = rate(culturalReligiousMet, totalWardrobeReviews);

  const dignityMaintained = wardrobe_review_records.filter(
    (r) => r.dignity_maintained,
  ).length;
  const dignityMaintainedRate = rate(dignityMaintained, totalWardrobeReviews);
  ...
  const reviewChildCoverage =
    total_children > 0 ? rate(uniqueChildrenReviewed, total_children) : 0;
  ...
  const inventoryCompletenessRate = rate(totalItemsAccountedFor, totalItemsRecorded);

  const totalMissing = personal_inventory_records.reduce(
    (sum, r) => sum + r.items_missing,
    0,
  );
  const missingRate = rate(totalMissing, totalItemsRecorded);

  const sentimentalSafeguarded = personal_inventory_records.filter(
    (r) => r.sentimental_items_safeguarded,
  ).length;
  const sentimentalSafeguardedRate = rate(sentimentalSafeguarded, totalInventoryRecords);
  ...
  const storageAdequateRate = rate(storageAdequate, totalInventoryRecords);

  const privacyRespected = personal_inventory_records.filter(
    (r) => r.privacy_respected,
  ).length;
  const privacyRespectedRate = rate(privacyRespected, totalInventoryRecords);

  const photographicRecord = personal_inventory_records.filter(
    (r) => r.photographic_record,
  ).length;
  const photographicRecordRate = rate(photographicRecord, totalInventoryRecords);
  ...
  const requestFulfilmentRate = rate(fulfilledRequests, totalRequestRecords);
  ...
  const urgentFulfilmentRate = rate(urgentFulfilled, urgentRequests);
  ...
  const childSatisfiedRequestRate = rate(childSatisfiedRequest, fulfilledRequests);
  ...
  const possessionSafeguardingRate = rate(resolvedIncidents, totalSafeguardingRecords);

  const documentedIncidents = possession_safeguarding_records.filter(
    (r) => r.incident_documented,
  ).length;
  const documentedRate = rate(documentedIncidents, totalSafeguardingRecords);
  ...
  const childChoiceRate = rate(totalChildChoiceNum, totalChildChoiceDenom);
  ...
  if (meets(allowanceUtilisationRate, 80)) score += 4;
  else if (meets(allowanceUtilisationRate, 60)) score += 2;

  // --- Bonus 2: wardrobeAdequacyRate (>=90: +4, >=70: +2) ---
  if (meets(wardrobeAdequacyRate, 90)) score += 4;
  else if (meets(wardrobeAdequacyRate, 70)) score += 2;

  // --- Bonus 3: inventoryCompletenessRate (>=95: +3, >=80: +1) ---
  if (meets(inventoryCompletenessRate, 95)) score += 3;
  else if (meets(inventoryCompletenessRate, 80)) score += 1;

  // --- Bonus 4: requestFulfilmentRate (>=90: +3, >=70: +1) ---
  if (meets(requestFulfilmentRate, 90)) score += 3;
  else if (meets(requestFulfilmentRate, 70)) score += 1;

  // --- Bonus 5: possessionSafeguardingRate (>=90: +3, >=70: +1) ---
  if (meets(possessionSafeguardingRate, 90)) score += 3;
  else if (meets(possessionSafeguardingRate, 70)) score += 1;
  ...
  if (meets(dignityMaintainedRate, 95)) score += 3;
  else if (meets(dignityMaintainedRate, 80)) score += 1;

  // --- Bonus 8: replacementRate (>=90: +3, >=70: +1) ---
  if (meets(replacementRate, 90)) score += 3;
  else if (meets(replacementRate, 70)) score += 1;
  ...
  if (below(wardrobeAdequacyRate, 50) && totalWardrobeReviews > 0) score -= 6;

  // requestFulfilmentRate < 50 → -5 (guarded)
  if (below(requestFulfilmentRate, 50) && totalRequestRecords > 0) score -= 5;

  // childChoiceRate < 30 → -3 (guarded)
  if (below(childChoiceRate, 30) && totalChildChoiceDenom > 0) score -= 3;

  // possessionSafeguardingRate < 50 → -4 (guarded)
  if (below(possessionSafeguardingRate, 50) && totalSafeguardingRecords > 0) score -= 4;
  ...
  if (meets(allowanceUtilisationRate, 80) && totalAllowanceRecords > 0) {
    strengths.push(
  ...
  } else if (meets(allowanceUtilisationRate, 60) && totalAllowanceRecords > 0) {
    strengths.push(
  ...
  if (meets(wardrobeAdequacyRate, 90) && totalWardrobeReviews > 0) {
    strengths.push(
  ...
  } else if (meets(wardrobeAdequacyRate, 70) && totalWardrobeReviews > 0) {
    strengths.push(
      `${wardrobeAdequacyRate}% wardrobe adequacy rate — the majority of wardrobe reviews confirm children have appropriate clothing.`,
    );
  }

  if (meets(inventoryCompletenessRate, 95) && totalInventoryRecords > 0) {
    strengths.push(
  ...
  } else if (meets(inventoryCompletenessRate, 80) && totalInventoryRecords > 0) {
    strengths.push(
      `${inventoryCompletenessRate}% inventory completeness rate — good tracking of children's personal possessions.`,
    );
  }

  if (meets(requestFulfilmentRate, 90) && totalRequestRecords > 0) {
    strengths.push(
  ...
  } else if (meets(requestFulfilmentRate, 70) && totalRequestRecords > 0) {
    strengths.push(
      `${requestFulfilmentRate}% clothing request fulfilment rate — the home generally meets children's clothing requests.`,
    );
  }

  if (meets(possessionSafeguardingRate, 90) && totalSafeguardingRecords > 0) {
    strengths.push(
  ...
  } else if (meets(possessionSafeguardingRate, 70) && totalSafeguardingRecords > 0) {
    strengths.push(
  ...
  if (meets(childChoiceRate, 90) && totalChildChoiceDenom > 0) {
    strengths.push(
  ...
  } else if (meets(childChoiceRate, 70) && totalChildChoiceDenom > 0) {
    strengths.push(
  ...
  if (meets(dignityMaintainedRate, 95) && totalWardrobeReviews > 0) {
    strengths.push(
  ...
  } else if (meets(dignityMaintainedRate, 80) && totalWardrobeReviews > 0) {
    strengths.push(
  ...
  } else if (meets(replacementRate, 70) && totalItemsNeedingReplacement > 0) {
    strengths.push(
  ...
  if (meets(childInvolvedShoppingRate, 90) && totalAllowanceRecords > 0) {
    strengths.push(
  ...
  if (meets(culturalReligiousRate, 95) && totalWardrobeReviews > 0) {
    strengths.push(
  ...
  if (meets(sentimentalSafeguardedRate, 90) && totalInventoryRecords > 0) {
    strengths.push(
  ...
  if (meets(urgentFulfilmentRate, 95) && urgentRequests > 0) {
    strengths.push(
  ...
  if (meets(childSatisfiedRate, 90) && totalWardrobeReviews > 0) {
    strengths.push(
  ...
  if (meets(seasonsCoveredCount, 4) && totalWardrobeReviews > 0) {
    strengths.push(
  ...
  if (meets(privacyRespectedRate, 95) && totalInventoryRecords > 0) {
    strengths.push(
  ...
  if (meets(storageAdequateRate, 90) && totalInventoryRecords > 0) {
    strengths.push(
  ...
  if (below(wardrobeAdequacyRate, 50) && totalWardrobeReviews > 0) {
    concerns.push(
  ...
  } else if (below(wardrobeAdequacyRate, 70) && meets(wardrobeAdequacyRate, 50) && totalWardrobeReviews > 0) {
    concerns.push(
  ...
  if (below(requestFulfilmentRate, 50) && totalRequestRecords > 0) {
    concerns.push(
  ...
  } else if (below(requestFulfilmentRate, 70) && meets(requestFulfilmentRate, 50) && totalRequestRecords > 0) {
    concerns.push(
  ...
  if (below(childChoiceRate, 30) && totalChildChoiceDenom > 0) {
    concerns.push(
  ...
  } else if (below(childChoiceRate, 70) && meets(childChoiceRate, 30) && totalChildChoiceDenom > 0) {
    concerns.push(
  ...
  if (below(possessionSafeguardingRate, 50) && totalSafeguardingRecords > 0) {
    concerns.push(
  ...
  } else if (below(possessionSafeguardingRate, 70) && meets(possessionSafeguardingRate, 50) && totalSafeguardingRecords > 0) {
    concerns.push(
  ...
  if (below(allowanceUtilisationRate, 40) && totalAllowanceRecords > 0) {
    concerns.push(
  ...
  } else if (below(allowanceUtilisationRate, 60) && meets(allowanceUtilisationRate, 40) && totalAllowanceRecords > 0) {
    concerns.push(
  ...
  if (below(inventoryCompletenessRate, 70) && totalInventoryRecords > 0) {
    concerns.push(
  ...
  } else if (below(inventoryCompletenessRate, 80) && meets(inventoryCompletenessRate, 70) && totalInventoryRecords > 0) {
    concerns.push(
  ...
  if (below(dignityMaintainedRate, 80) && totalWardrobeReviews > 0) {
    concerns.push(
  ...
  if (below(culturalReligiousRate, 80) && totalWardrobeReviews > 0) {
    concerns.push(
  ...
  if (meets(missingRate, 15) && totalInventoryRecords > 0) {
    concerns.push(
  ...
  } else if (meets(missingRate, 8) && below(missingRate, 15) && totalInventoryRecords > 0) {
    concerns.push(
  ...
  if (below(urgentFulfilmentRate, 80) && urgentRequests > 0) {
    concerns.push(
  ...
  if (below(reviewChildCoverage, 80) && total_children > 0 && totalWardrobeReviews > 0) {
    concerns.push(
  ...
  if (below(storageAdequateRate, 70) && totalInventoryRecords > 0) {
    concerns.push(
  ...
  if (below(childSatisfiedRequestRate, 60) && fulfilledRequests > 0) {
    concerns.push(
  ...
  } else if (below(replacementRate, 70) && meets(replacementRate, 50) && totalItemsNeedingReplacement > 0) {
    concerns.push(
  ...
  if (
    meets(allowanceUtilisationRate, 40) &&
    below(allowanceUtilisationRate, 60) &&
    totalAllowanceRecords > 0
  ) {
    recommendations.push({
      rank: ++rank,
  ...
  if (
    meets(requestFulfilmentRate, 50) &&
    below(requestFulfilmentRate, 70) &&
  ...
  if (
    meets(possessionSafeguardingRate, 50) &&
    below(possessionSafeguardingRate, 70) &&
  ...
  if (below(replacementRate, 70) && totalItemsNeedingReplacement > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (
    meets(wardrobeAdequacyRate, 50) &&
    below(wardrobeAdequacyRate, 70) &&
  ...
  if (
    meets(inventoryCompletenessRate, 70) &&
    below(inventoryCompletenessRate, 80) &&
  ...
  if (
    meets(childSatisfiedRequestRate, 40) &&
    below(childSatisfiedRequestRate, 60) &&
    fulfilledRequests > 0
  ) {
    recommendations.push({
      rank: ++rank,
  ...
  if (below(photographicRecordRate, 50) && totalInventoryRecords > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (below(dignityMaintainedRate, 70) && totalWardrobeReviews > 0) {
    insights.push({
  ...
  if (
    meets(replacementRate, 50) &&
    below(replacementRate, 70) &&
  ...
  if (
    meets(wardrobeAdequacyRate, 90) &&
    meets(dignityMaintainedRate, 95) &&
    totalWardrobeReviews > 0
  ) {
    insights.push({
  ...
  if (
    meets(requestFulfilmentRate, 90) &&
    meets(childSatisfiedRequestRate, 85) &&
    totalRequestRecords > 0 &&
    fulfilledRequests > 0
  ) {
    insights.push({
  ...
  if (
    meets(inventoryCompletenessRate, 95) &&
    meets(sentimentalSafeguardedRate, 90) &&
    totalInventoryRecords > 0
  ) {
    insights.push({
  ...
  if (
    meets(possessionSafeguardingRate, 90) &&
    meets(documentedRate, 90) &&
    totalSafeguardingRecords > 0
  ) {
    insights.push({
  ...
  if (
    meets(allowanceChildCoverage, 100) &&
    meets(childInvolvedShoppingRate, 90) &&
    total_children > 0 &&
    totalAllowanceRecords > 0
  ) {
    insights.push({
  ...
  if (
    meets(culturalReligiousRate, 95) &&
    totalWardrobeReviews > 0
  ) {
    insights.push({
