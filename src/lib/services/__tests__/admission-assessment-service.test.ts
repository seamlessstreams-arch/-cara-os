...
function makeRecord(overrides?: Partial<AdmissionAssessmentRecord>): AdmissionAssessmentRecord {
  return {
    ...
    impact_risk_completed: true,
    matching_criteria_met: true,
    existing_children_consulted: true,
    pre_admission_visit_completed: true,
    care_plan_received: true,
    health_assessment_available: true,
    education_info_received: true,
    risk_assessments_reviewed: true,
    safeguarding_info_shared: true,
    placement_plan_agreed: true,
...
describe("admission-assessment-service", () => {
  describe("computeAdmissionAssessmentMetrics", () => {
    it("returns zeros for empty", () => { 
      const m = computeAdmissionAssessmentMetrics([]);
      expect(m.total_assessments).toBe(0);;
      expect(m.suitable_count).toBe(0);
      expect(m.unsuitable_count).toBe(0);
      expect(m.pending_count).toBe(0);
      expect(m.impact_risk_rate).toBeNull();;
      expect(m.unique_children).toBe(0);
    });
    it("returns empty breakdowns", () => { 
      const m = computeAdmissionAssessmentMetrics([]);
      expect(m.by_assessment_stage).toEqual({});
      expect(m.by_suitability_decision).toEqual({});
      expect(m.by_matching_outcome).toEqual({});
      expect(m.by_referral_source).toEqual({});
    });
    ...
    it("returns 100% boolean rates with defaults", () => { 
      const m = computeAdmissionAssessmentMetrics([makeRecord()]);
      expect(m.impact_risk_rate).toBe(100);
      expect(m.matching_criteria_rate).toBe(100);
      expect(m.existing_children_consulted_rate).toBe(100);
      expect(m.pre_admission_visit_rate).toBe(100);
      expect(m.care_plan_received_rate).toBe(100);
      expect(m.health_assessment_rate).toBe(100);
      expect(m.education_info_rate).toBe(100);
      expect(m.risk_assessments_rate).toBe(100);
      expect(m.safeguarding_shared_rate).toBe(100);
      expect(m.placement_plan_rate).toBe(100);
      expect(m.key_worker_rate).toBe(100);
      expect(m.bedroom_prepared_rate).toBe(100);
    });
  ...
  describe("identifyAdmissionAssessmentAlerts", () => {
    ...
    it("fires poor_match_admitted", () => { 
      const a = identifyAdmissionAssessmentAlerts([makeRecord({ matching_outcome: "poor_match", suitability_decision: "suitable", child_name: "X", assessment_date: "2026-05-14" })]);
      expect(a[0].type).toBe("poor_match_admitted");
    ...
    it("fires impact_risk_incomplete singular", () => { 
      const a = identifyAdmissionAssessmentAlerts([makeRecord({ impact_risk_completed: false })]);
      const f = a.find(x => x.type === "impact_risk_incomplete");
    ...
    it("fires all applicable", () => { 
      const a = identifyAdmissionAssessmentAlerts([makeRecord({ matching_outcome: "poor_match", suitability_decision: "suitable", impact_risk_completed: false, existing_children_consulted: false, care_plan_received: false, key_worker_allocated: false }), makeRecord({ care_plan_received: false, key_worker_allocated: false })]);
      const types = a.map(x => x.type);
      expect(types).toContain("poor_match_admitted");
      expect(types).toContain("impact_risk_incomplete");
      expect(types).toContain("children_not_consulted");
      expect(types).toContain("care_plan_missing");
      expect(types).toContain("key_worker_not_allocated");
    });
  });
});
