...
// ── Data hooks (inlined from the former use-livers wrapper) ───────────────────

type ListResponse<T> = { data: T[]; meta: Record<string, unknown> };
type SingleResponse<T> = { data: T };
type RolePayload = { user_role?: string };

function useLiversAnalyses(params?: { childId?: string; homeId?: string }) {
...
function useCreateLiversAnalysis() {
...
function useUpdateLiversAnalysis() {
  const qc = useQueryClient();
  return useMutation({
...
function useInterventionSessions(params?: {
  childId?: string;
  homeId?: string;
  liversId?: string;
}) {
...
function useCreateInterventionSession() {
  const qc = useQueryClient();
  return useMutation({
...
function useCreateLiversOutcome() {
  const qc = useQueryClient();
  return useMutation({
