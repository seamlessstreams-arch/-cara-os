...
/** Just the collections this mapper reads — callers assemble a subset of the
 *  store, not the whole thing. */
export type EventStoreShape = Pick<
...
export function mapStoreToEventInput(store: EventStoreShape): EventProjectorInput {
  return {
    homeId: "home_oak",
    ...
    missingEpisodes: (((store.missingEpisodes ?? []))).map((m) => ({
    ...
    dailyLogs: (((store.dailyLog ?? []))).map((l) => ({
    ...
    keyworkSessions: (((store.keyWorkingSessions ?? []))).map((k) => ({
    ...
    educationRecords: (((store.educationRecords ?? []))).map((e) => ({
    ...
    maintenance: (((store.maintenance ?? []))).map((m) => ({
    ...
    reg44Reports: (((store.reg44VisitReports ?? []))).map((r) => ({
    ...
    appointments: (((store.appointments ?? []))).map((a) => ({
      ...
      status: a.status, outcome: a.outcome, recorded_by: a.recorded_by, created_at: a.created_at,
    })),
    leaveRequests: (((store.leaveRequests ?? []))).map((l) => ({
    ...
    complaints: (((store.complaints ?? []))).map((c) => ({
    ...
    familyContacts: (((intelligenceDb.contactLogs.findAll("home_oak") ?? []))).map((c) => ({
    ...
    riskAssessments: (((store.riskAssessments ?? []))).map((r) => ({
    ...
    notifiableEvents: (((store.notifiableEvents ?? []))).map((r) => ({
      ...
      reported_by: r.reported_by, ofsted_status: r.ofsted_status,
    })),
    behaviourSupportPlans: (((store.behaviourSupportPlans ?? []))).map((r) => ({
      ...
      primary_behaviours: (((r.primary_behaviours ?? []))).map((b) => ({ severity: b.severity })),
