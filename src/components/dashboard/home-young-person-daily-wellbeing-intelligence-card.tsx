...
import type { YPDailyWellbeingRating, YoungPersonDailyWellbeingResult } from "@/lib/engines/home-young-person-daily-wellbeing-intelligence-engine";
...
export function HomeYoungPersonDailyWellbeingIntelligenceCard() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: YoungPersonDailyWellbeingResult }>;
    },
      ...
      <CardContent className="space-y-4">
        {d.wellbeing_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.average_mood_score === null ? "bg-slate-50" : d.average_mood_score >= 7 ? "bg-green-50" : d.average_mood_score >= 5 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.average_mood_score === null ? "text-muted-foreground" : d.average_mood_score >= 7 ? "text-[--cs-success]" : d.average_mood_score >= 5 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.average_mood_score}</p>
              <p className="text-[9px] text-muted-foreground">Mood</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.daily_coverage_rate === null ? "bg-slate-50" : d.daily_coverage_rate >= 90 ? "bg-green-50" : d.daily_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.daily_coverage_rate === null ? "text-muted-foreground" : d.daily_coverage_rate >= 90 ? "text-[--cs-success]" : d.daily_coverage_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.daily_coverage_rate)}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_coverage_rate === null ? "bg-slate-50" : d.child_coverage_rate >= 100 ? "bg-green-50" : d.child_coverage_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_coverage_rate === null ? "text-muted-foreground" : d.child_coverage_rate >= 100 ? "text-[--cs-success]" : d.child_coverage_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.child_coverage_rate)}</p>
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Daily Wellbeing Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
