  ...
  describe("computeCleaningMetrics", () => {
    describe("empty records", () => {
      it("returns zeros for all fields", () => {
        ...
        expect(m.products_safe_rate).toBeNull();;
        expect(m.surfaces_sanitised_rate).toBeNull();;
        expect(m.waste_disposed_rate).toBeNull();;
        expect(m.hand_washing_rate).toBeNull();;
        expect(m.ventilation_rate).toBeNull();;
