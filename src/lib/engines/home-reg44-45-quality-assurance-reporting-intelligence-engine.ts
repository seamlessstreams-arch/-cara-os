...
export interface Reg4445QualityAssuranceReportingResult {
  ...
  reg44_completion_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  reg45_timeliness_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  action_plan_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  quality_improvement_rate: number | null;
  // fab-0: null when no notifications logged.
  notification_compliance_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  stakeholder_engagement_rate: number | null;
...
function emptyResult(
  ...
  return {
    ...
    reg44_completion_rate: null,
    reg45_timeliness_rate: null,
    action_plan_rate: null,
    quality_improvement_rate: null,
    notification_compliance_rate: null,
    stakeholder_engagement_rate: null,
...
export function computeReg4445QualityAssuranceReporting(
  ...
  const reg44CompletionRate = rate(submittedReg44, totalReg44);

  const independentVisitors = reg44_report_records.filter(
    (r) => r.visitor_independent,
  ).length;
  const independenceRate = rate(independentVisitors, totalReg44);

  const childViewsCaptured = reg44_report_records.filter(
    (r) => r.child_views_captured,
  ).length;
  const reg44ChildViewsRate = rate(childViewsCaptured, totalReg44);
  ...
  const shortfallActionRate = rate(totalShortfallsActioned, totalShortfallsIdentified);
  ...
  const prevActionResolutionRate = rate(totalPrevActionsResolved, totalPrevActionsTotal);
  ...
  const childrenSpokenToRate = rate(totalChildrenSpokenTo, totalChildrenAvailable);
  ...
  const reg45TimelinessRate = rate(onTimeReg45, totalReg45);

  const coversAllStandards = reg45_review_records.filter(
    (r) => r.review_covers_all_standards,
  ).length;
  const reg45CoverageRate = rate(coversAllStandards, totalReg45);

  const devPlanUpdated = reg45_review_records.filter(
    (r) => r.development_plan_updated,
  ).length;
  const devPlanUpdateRate = rate(devPlanUpdated, totalReg45);
  ...
  const reg45ChildConsultationRate = rate(totalChildrenConsultedReg45, totalChildrenTotalReg45);
  ...
  const reg44ConsideredInReg45Rate = rate(totalReg44Considered, totalReg44Available);
  ...
  const stakeholderEngagementRate = rate(totalStakeholderTouchpoints, maxStakeholderTouchpoints);
  ...
  const actionPlanRate = rate(completedActions, actionableActions);

  const verifiedByManager = action_plan_records.filter(
    (a) => a.status === "completed" && a.verified_by_manager,
  ).length;
  const managerVerificationRate = rate(verifiedByManager, completedActions);

  const evidenceOfCompletion = action_plan_records.filter(
    (a) => a.status === "completed" && a.evidence_of_completion,
  ).length;
  const evidenceRate = rate(evidenceOfCompletion, completedActions);
  ...
  const followUpRate = rate(followUpCompleted, followUpRequired);
  ...
  const qualityImprovementRate = rate(improvementAchieved, totalQualityCycles);

  const sustainedImprovement = quality_improvement_records.filter(
    (q) => q.improvement_achieved && q.sustained_over_time,
  ).length;
  const sustainedImprovementRate = rate(sustainedImprovement, improvementAchieved);

  const outcomeMeasured = quality_improvement_records.filter(
    (q) => q.outcome_measured,
  ).length;
  const outcomeMeasurementRate = rate(outcomeMeasured, totalQualityCycles);

  const evidenceCollected = quality_improvement_records.filter(
    (q) => q.evidence_collected,
  ).length;
  const evidenceCollectionRate = rate(evidenceCollected, totalQualityCycles);

  const childrenConsultedInCycles = quality_improvement_records.filter(
    (q) => q.children_consulted,
  ).length;
  const cycleChildConsultationRate = rate(childrenConsultedInCycles, totalQualityCycles);
  ...
  const notificationTimelinessRate = rate(notifiedWithin24hrs, totalNotifications);

  const notifiedOfsted = notification_records.filter(
    (n) => n.notified_ofsted,
  ).length;
  const ofstedNotificationRate = rate(notifiedOfsted, totalNotifications);

  const investigationsCompleted = notification_records.filter(
    (n) => n.investigation_completed,
  ).length;
  const investigationCompletionRate = rate(investigationsCompleted, totalNotifications);

  const childInformedOfOutcome = notification_records.filter(
    (n) => n.child_informed_of_outcome,
  ).length;
  const childInformedRate = rate(childInformedOfOutcome, totalNotifications);

  const documentedInRecords = notification_records.filter(
    (n) => n.documented_in_records,
  ).length;
  const documentationRate = rate(documentedInRecords, totalNotifications);
  ...
  const notificationActionRate = rate(notificationActionsCompleted, notificationActionsTotal);

  // Composite notification compliance
  // fab-0: null when no notifications recorded.
  const notificationComplianceRate: number | null =
    totalNotifications > 0 ? Math.round(
          (notificationTimelinessRate! + ofstedNotificationRate! + documentationRate!) / 3) : null;
  ...
  if (meets(reg44CompletionRate, 100)) score += 5;
  else if (meets(reg44CompletionRate, 80)) score += 3;

  // --- Bonus 2: reg45TimelinessRate (>=100: +5, >=80: +3) ---
  if (meets(reg45TimelinessRate, 100)) score += 5;
  else if (meets(reg45TimelinessRate, 80)) score += 3;

  // --- Bonus 3: actionPlanRate (>=90: +5, >=70: +3) ---
  if (meets(actionPlanRate, 90)) score += 5;
  else if (meets(actionPlanRate, 70)) score += 3;

  // --- Bonus 4: qualityImprovementRate (>=80: +4, >=60: +2) ---
  if (meets(qualityImprovementRate, 80)) score += 4;
  else if (meets(qualityImprovementRate, 60)) score += 2;
  ...
  if (meets(stakeholderEngagementRate, 80)) score += 4;
  else if (meets(stakeholderEngagementRate, 60)) score += 2;

  // ── Penalties (4 guarded) ─────────────────────────────────────────────

  // Penalty 1: reg44CompletionRate < 50 → -6
  if (below(reg44CompletionRate, 50) && totalReg44 > 0) score -= 6;

  // Penalty 2: reg45TimelinessRate < 50 → -6
  if (below(reg45TimelinessRate, 50) && totalReg45 > 0) score -= 6;

  // Penalty 3: actionPlanRate < 40 → -5
  if (below(actionPlanRate, 40) && actionableActions > 0) score -= 5;
  ...
  if (meets(reg44CompletionRate, 100) && totalReg44 > 0) {
    strengths.push(
  ...
  } else if (meets(actionPlanRate, 70) && actionableActions > 0) {
    strengths.push(
      `${actionPlanRate}% action plan completion — the majority of identified actions are being addressed and closed.`,
    );
  }

  if (meets(qualityImprovementRate, 80) && totalQualityCycles > 0) {
    strengths.push(
  ...
  } else if (meets(qualityImprovementRate, 60) && totalQualityCycles > 0) {
    strengths.push(
  ...
  if (meets(stakeholderEngagementRate, 80) && totalReg45 > 0) {
    strengths.push(
  ...
  } else if (meets(stakeholderEngagementRate, 60) && totalReg45 > 0) {
    strengths.push(
  ...
  if (meets(independenceRate, 100) && totalReg44 > 0) {
    strengths.push(
  ...
  if (meets(reg44ChildViewsRate, 90) && totalReg44 > 0) {
    strengths.push(
  ...
  if (meets(childrenSpokenToRate, 90) && totalChildrenAvailable > 0) {
    strengths.push(
  ...
  if (meets(shortfallActionRate, 90) && totalShortfallsIdentified > 0) {
    strengths.push(
      `${shortfallActionRate}% of Reg 44 shortfalls actioned — the home responds promptly to independent visitor findings.`,
    );
  }

  if (meets(prevActionResolutionRate, 90) && totalPrevActionsTotal > 0) {
    strengths.push(
  ...
  if (meets(reg45CoverageRate, 100) && totalReg45 > 0) {
    strengths.push(
  ...
  if (meets(devPlanUpdateRate, 100) && totalReg45 > 0) {
    strengths.push(
  ...
  if (meets(reg44ConsideredInReg45Rate, 100) && totalReg44Available > 0) {
    strengths.push(
  ...
  if (meets(sustainedImprovementRate, 80) && improvementAchieved > 0) {
    strengths.push(
  ...
  if (meets(recordsCheckRate, 90) && totalReg44 > 0) {
    strengths.push(
  ...
  if (meets(reg45ChildConsultationRate, 90) && totalChildrenTotalReg45 > 0) {
    strengths.push(
  ...
  if (below(reg44CompletionRate, 50) && totalReg44 > 0) {
    concerns.push(
  ...
  } else if (below(reg44CompletionRate, 80) && meets(reg44CompletionRate, 50) && totalReg44 > 0) {
    concerns.push(
  ...
  if (below(reg45TimelinessRate, 50) && totalReg45 > 0) {
    concerns.push(
  ...
  } else if (below(reg45TimelinessRate, 80) && meets(reg45TimelinessRate, 50) && totalReg45 > 0) {
    concerns.push(
  ...
  if (below(actionPlanRate, 40) && actionableActions > 0) {
    concerns.push(
  ...
  } else if (below(actionPlanRate, 70) && meets(actionPlanRate, 40) && actionableActions > 0) {
    concerns.push(
  ...
  if (below(qualityImprovementRate, 40) && totalQualityCycles > 0) {
    concerns.push(
  ...
  } else if (below(qualityImprovementRate, 60) && meets(qualityImprovementRate, 40) && totalQualityCycles > 0) {
    concerns.push(
  ...
  if (below(notificationTimelinessRate, 80) && totalNotifications > 0) {
    concerns.push(
  ...
  if (below(stakeholderEngagementRate, 40) && totalReg45 > 0) {
    concerns.push(
  ...
  } else if (below(stakeholderEngagementRate, 60) && meets(stakeholderEngagementRate, 40) && totalReg45 > 0) {
    concerns.push(
  ...
  if (below(independenceRate, 80) && totalReg44 > 0) {
    concerns.push(
  ...
  if (below(reg44ChildViewsRate, 70) && totalReg44 > 0) {
    concerns.push(
  ...
  if (below(shortfallActionRate, 60) && totalShortfallsIdentified > 0) {
    concerns.push(
  ...
  if (below(reg45CoverageRate, 80) && totalReg45 > 0) {
    concerns.push(
  ...
  if (below(reg44ConsideredInReg45Rate, 80) && totalReg44Available > 0) {
    concerns.push(
  ...
  if (below(reg45ChildConsultationRate, 70) && totalChildrenTotalReg45 > 0) {
    concerns.push(
  ...
  if (below(managerVerificationRate, 60) && completedActions > 0) {
    concerns.push(
  ...
  if (below(evidenceRate, 60) && completedActions > 0) {
    concerns.push(
  ...
  if (below(followUpRate, 70) && followUpRequired > 0) {
    concerns.push(
  ...
  if (below(investigationCompletionRate, 80) && totalNotifications > 0) {
    concerns.push(
  ...
  if (below(recordsCheckRate, 70) && totalReg44 > 0) {
    concerns.push(
  ...
  if (below(sustainedImprovementRate, 50) && improvementAchieved > 0) {
    concerns.push(
  ...
  if (meets(reg44CompletionRate, 50) && below(reg44CompletionRate, 80) && totalReg44 > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (meets(reg45TimelinessRate, 50) && below(reg45TimelinessRate, 80) && totalReg45 > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (meets(actionPlanRate, 40) && below(actionPlanRate, 70) && actionableActions > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (below(qualityImprovementRate, 60) && totalQualityCycles > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (below(stakeholderEngagementRate, 60) && totalReg45 > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (below(devPlanUpdateRate, 80) && totalReg45 > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (below(cycleChildConsultationRate, 60) && totalQualityCycles > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (below(childInformedRate, 80) && totalNotifications > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (below(independenceRate, 50) && totalReg44 > 0) {
    insights.push({
  ...
  if (overdueActions > 0 && meets(actionPlanRate, 40) && totalActionPlans > 0) {
    insights.push({
  ...
  if (meets(qualityImprovementRate, 40) && below(qualityImprovementRate, 60) && totalQualityCycles > 0) {
    insights.push({
  ...
  if (meets(stakeholderEngagementRate, 40) && below(stakeholderEngagementRate, 60) && totalReg45 > 0) {
    insights.push({
  ...
  if (meets(independenceRate, 50) && below(independenceRate, 80) && totalReg44 > 0) {
    insights.push({
  ...
  if (meets(shortfallActionRate, 60) && below(shortfallActionRate, 80) && totalShortfallsIdentified > 0) {
    insights.push({
  ...
  if (meets(prevActionResolutionRate, 50) && below(prevActionResolutionRate, 80) && totalPrevActionsTotal > 0) {
    insights.push({
  ...
  if (below(devPlanUpdateRate, 80) && above(devPlanUpdateRate, 0) && totalReg45 > 0) {
    insights.push({
  ...
  if (meets(sustainedImprovementRate, 30) && below(sustainedImprovementRate, 50) && improvementAchieved > 0) {
    insights.push({
  ...
  if (
    meets(reg45TimelinessRate, 100) &&
    meets(reg45CoverageRate, 100) &&
    totalReg45 > 0
  ) {
    insights.push({
  ...
  if (
    meets(actionPlanRate, 90) &&
    meets(managerVerificationRate, 90) &&
    meets(evidenceRate, 90) &&
    actionableActions > 0
  ) {
    insights.push({
  ...
  if (
    meets(qualityImprovementRate, 80) &&
    meets(sustainedImprovementRate, 80) &&
  ...
  if (
    ...
    meets(investigationCompletionRate, 100)
  ) {
    insights.push({
  ...
  if (
    meets(stakeholderEngagementRate, 80) &&
    meets(reg45ChildConsultationRate, 90) &&
    totalReg45 > 0
  ) {
    insights.push({
  ...
  if (
    meets(reg44ConsideredInReg45Rate, 100) &&
    meets(devPlanUpdateRate, 100) &&
  ...
  if (
    meets(shortfallActionRate, 90) &&
    meets(prevActionResolutionRate, 90) &&
  ...
  if (
    meets(independenceRate, 100) &&
    meets(reg44ChildViewsRate, 90) &&
    meets(recordsCheckRate, 90) &&
    totalReg44 > 0
  ) {
    insights.push({
  ...
  if (
    meets(cycleChildConsultationRate, 80) &&
    meets(evidenceCollectionRate, 90) &&
    meets(outcomeMeasurementRate, 90) &&
    totalQualityCycles > 0
  ) {
    insights.push({
  ...
  if (
    meets(childInformedRate, 90) &&
    meets(notificationActionRate, 90) &&
    totalNotifications > 0
  ) {
    insights.push({
