...
import { GET, PATCH } from "@/app/api/intelligence/aria-suggestions/route";
...
describe("aria-suggestions route (fallback mode)", () => {
  ...
  it("GET returns all suggestions sorted by created_at desc", async () => {
    const res = await GET(makeReq("http://x/api/intelligence/aria-suggestions"));
  ...
  it("GET filters by status", async () => {
    const res = await GET(makeReq("http://x/api/intelligence/aria-suggestions?status=awaiting_review"));
    const body = await res.json();
    expect(body.ok).toBe(true);
  ...
  it("GET ?id= returns a single item", async () => {
    ...
    const res = await GET(makeReq(`http://x/api/intelligence/aria-suggestions?id=${seed.id}`));
    const body = await res.json();
    expect(body.ok).toBe(true);
  ...
  it("PATCH requires id and status", async () => {
    const res = await PATCH(
      makeReq("http://x/api/intelligence/aria-suggestions", {
        method: "PATCH",
        body: JSON.stringify({}),
      }),
    );
