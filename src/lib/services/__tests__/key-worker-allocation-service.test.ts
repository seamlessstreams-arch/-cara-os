...
describe("key-worker-allocation-service", () => {
  describe("computeKeyWorkerAllocationMetrics", () => {
    it("returns zeros for empty", () => { 
      ...
      expect(m.no_continuity_count).toBe(0);
      expect(m.child_views_rate).toBeNull();;
      expect(m.unique_children).toBe(0);
    });
    it("returns empty breakdowns", () => { 
      const m = computeKeyWorkerAllocationMetrics([]);
      expect(m.by_allocation_status).toEqual({});
      expect(m.by_relationship_quality).toEqual({});
      expect(m.by_workload_level).toEqual({});
      expect(m.by_continuity_rating).toEqual({});
    });
    ...
    it("returns 100% boolean rates with defaults", () => { 
      const m = computeKeyWorkerAllocationMetrics([makeRecord()]);
      ...
      expect(m.backup_worker_rate).toBe(100);
      expect(m.social_worker_rate).toBe(100);
      expect(m.relationship_supported_rate).toBe(100);
  ...
  describe("identifyKeyWorkerAllocationAlerts", () => {
    ...
    it("fires unallocated_broken_down", () => { 
      const a = identifyKeyWorkerAllocationAlerts([makeRecord({ allocation_status: "unallocated", relationship_quality: "broken_down", child_name: "Jo" })]);
      expect(a[0].type).toBe("unallocated_broken_down");
    ...
    it("fires children_unallocated singular", () => { 
      const a = identifyKeyWorkerAllocationAlerts([makeRecord({ allocation_status: "unallocated" })]);
      const f = a.find(x => x.type === "children_unallocated");
      expect(f).toBeDefined();
      expect(f!.severity).toBe("high");
      expect(f!.message).toContain("1 child is");
    });
    ...
    it("fires no_regular_sessions singular", () => { 
      const a = identifyKeyWorkerAllocationAlerts([makeRecord({ regular_sessions_held: false })]);
      const f = a.find(x => x.type === "no_regular_sessions");
      expect(f).toBeDefined();
      expect(f!.severity).toBe("high");
      expect(f!.message).toContain("1 allocation has");
    });
