...
export interface MultiAgencyCollaborationResult {
  ...
  lac_review_timeliness_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  social_worker_visit_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  therapeutic_engagement_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  education_liaison_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
...
function emptyResult(
  ...
  return {
    ...
    lac_review_timeliness_rate: null,
    social_worker_visit_rate: null,
    therapeutic_engagement_rate: null,
    education_liaison_rate: null,
    information_sharing_rate: null,
    multi_agency_meeting_rate: null,
    strengths: [],
    concerns: [],
    recommendations: [],
    insights: [],
...
export function computeMultiAgencyCollaboration(
  ...
  const lacReviewTimelinessRate = rate(onTimeLacReviews, totalLacReviews);

  const lacChildViewsRecorded = lac_review_records.filter((r) => r.child_views_recorded).length;
  const lacChildViewsRate = rate(lacChildViewsRecorded, totalLacReviews);
  ...
  const lacActionCompletionRate = rate(lacActionsCompleted, lacActionsSet);

  const lacPoorOutcomes = lac_review_records.filter((r) => r.outcome_quality === "poor").length;
  const lacPoorOutcomeRate = rate(lacPoorOutcomes, totalLacReviews);
  ...
  const therapeuticEngagementRate = rate(therapeuticEngaged, totalTherapeuticRecords);

  const therapeuticProgressReported = therapeutic_service_records.filter((t) => t.progress_reported).length;
  const therapeuticProgressRate = rate(therapeuticProgressReported, totalTherapeuticRecords);

  const onWaitingList = therapeutic_service_records.filter((t) => t.waiting_list).length;
  const waitingListRate = rate(onWaitingList, totalTherapeuticRecords);
  ...
  const therapeuticInfoSharedRate = rate(therapeuticInfoShared, totalTherapeuticRecords);
  ...
  const educationLiaisonRate = rate(attendedByHome, totalEducationLiaisons);

  const pepUpToDate = education_liaison_records.filter((e) => e.pep_up_to_date).length;
  const pepUpToDateRate = rate(pepUpToDate, totalEducationLiaisons);

  const designatedTeacherInvolved = education_liaison_records.filter((e) => e.designated_teacher_involved).length;
  const designatedTeacherRate = rate(designatedTeacherInvolved, totalEducationLiaisons);
  ...
  const informationSharingRate = rate(timelySharing, totalInfoSharingRecords);

  const gdprCompliant = information_sharing_records.filter((i) => i.gdpr_compliant).length;
  const gdprComplianceRate = rate(gdprCompliant, totalInfoSharingRecords);

  const homeInitiated = information_sharing_records.filter((i) => i.initiated_by_home).length;
  const homeInitiatedRate = rate(homeInitiated, totalInfoSharingRecords);
  ...
  const multiAgencyMeetingRate = rate(multiAgencyMeetings, totalInfoSharingRecords);
  ...
  if (meets(lacReviewTimelinessRate, 95)) score += 4;
  else if (meets(lacReviewTimelinessRate, 80)) score += 2;

  // --- Bonus 2: socialWorkerVisitRate (>=95: +4, >=80: +2) ---
  if (meets(socialWorkerVisitRate, 95)) score += 4;
  else if (meets(socialWorkerVisitRate, 80)) score += 2;

  // --- Bonus 3: therapeuticEngagementRate (>=90: +3, >=70: +1) ---
  if (meets(therapeuticEngagementRate, 90)) score += 3;
  else if (meets(therapeuticEngagementRate, 70)) score += 1;

  // --- Bonus 4: educationLiaisonRate (>=95: +3, >=80: +1) ---
  if (meets(educationLiaisonRate, 95)) score += 3;
  else if (meets(educationLiaisonRate, 80)) score += 1;

  // --- Bonus 5: informationSharingRate (>=95: +3, >=80: +1) ---
  if (meets(informationSharingRate, 95)) score += 3;
  else if (meets(informationSharingRate, 80)) score += 1;

  // --- Bonus 6: lacActionCompletionRate (>=90: +3, >=70: +1) ---
  if (meets(lacActionCompletionRate, 90)) score += 3;
  else if (meets(lacActionCompletionRate, 70)) score += 1;

  // --- Bonus 7: childSeenAloneRate (>=95: +3, >=80: +1) ---
  if (meets(childSeenAloneRate, 95)) score += 3;
  else if (meets(childSeenAloneRate, 80)) score += 1;

  // --- Bonus 8: pepUpToDateRate (>=95: +3, >=70: +1) ---
  if (meets(pepUpToDateRate, 95)) score += 3;
  else if (meets(pepUpToDateRate, 70)) score += 1;

  // --- Bonus 9: gdprComplianceRate (>=95: +2, >=80: +1) ---
  if (meets(gdprComplianceRate, 95)) score += 2;
  else if (meets(gdprComplianceRate, 80)) score += 1;

  // ── Penalties ─────────────────────────────────────────────────────────

  // lacReviewTimelinessRate < 50 -> -6 (guarded)
  if (below(lacReviewTimelinessRate, 50) && totalLacReviews > 0) score -= 6;

  // socialWorkerVisitRate < 50 -> -5 (guarded)
  if (below(socialWorkerVisitRate, 50) && totalSWVisits > 0) score -= 5;

  // therapeuticEngagementRate < 40 -> -4 (guarded)
  if (below(therapeuticEngagementRate, 40) && totalTherapeuticRecords > 0) score -= 4;

  // informationSharingRate < 50 -> -3 (guarded)
  if (below(informationSharingRate, 50) && totalInfoSharingRecords > 0) score -= 3;
  ...
  if (meets(lacReviewTimelinessRate, 95) && totalLacReviews > 0) {
    strengths.push(
  ...
  } else if (meets(lacReviewTimelinessRate, 80) && totalLacReviews > 0) {
    strengths.push(
  ...
  if (meets(socialWorkerVisitRate, 95) && totalSWVisits > 0) {
    strengths.push(
  ...
  } else if (meets(socialWorkerVisitRate, 80) && totalSWVisits > 0) {
    strengths.push(
  ...
  if (meets(therapeuticEngagementRate, 90) && totalTherapeuticRecords > 0) {
    strengths.push(
  ...
  } else if (meets(therapeuticEngagementRate, 70) && totalTherapeuticRecords > 0) {
    strengths.push(
  ...
  if (meets(educationLiaisonRate, 95) && totalEducationLiaisons > 0) {
    strengths.push(
  ...
  } else if (meets(educationLiaisonRate, 80) && totalEducationLiaisons > 0) {
    strengths.push(
  ...
  if (meets(informationSharingRate, 95) && totalInfoSharingRecords > 0) {
    strengths.push(
  ...
  } else if (meets(informationSharingRate, 80) && totalInfoSharingRecords > 0) {
    strengths.push(
  ...
  if (meets(lacActionCompletionRate, 90) && lacActionsSet > 0) {
    strengths.push(
  ...
  } else if (meets(lacActionCompletionRate, 70) && lacActionsSet > 0) {
    strengths.push(
  ...
  // S7: Child seen alone
  if (meets(childSeenAloneRate, 95) && totalSWVisits > 0) {
    strengths.push(
  ...
  } else if (meets(childSeenAloneRate, 80) && totalSWVisits > 0) {
    strengths.push(
  ...
  // S8: PEP up to date
  if (meets(pepUpToDateRate, 95) && totalEducationLiaisons > 0) {
    strengths.push(
  ...
  } else if (meets(pepUpToDateRate, 70) && totalEducationLiaisons > 0) {
    strengths.push(
  ...
  // S9: GDPR compliance
  if (meets(gdprComplianceRate, 95) && totalInfoSharingRecords > 0) {
    strengths.push(
  ...
  } else if (meets(gdprComplianceRate, 80) && totalInfoSharingRecords > 0) {
    strengths.push(
  ...
  if (meets(swConsistencyRate, 90) && totalSWVisits > 0) {
    strengths.push(
  ...
  if (meets(lacChildViewsRate, 95) && totalLacReviews > 0) {
    strengths.push(
  ...
  if (meets(therapeuticProgressRate, 90) && totalTherapeuticRecords > 0) {
    strengths.push(
  ...
  if (meets(homeInitiatedRate, 70) && totalInfoSharingRecords > 0) {
    strengths.push(
  ...
  if (meets(designatedTeacherRate, 90) && totalEducationLiaisons > 0) {
    strengths.push(
  ...
  if (below(lacReviewTimelinessRate, 50) && totalLacReviews > 0) {
    concerns.push(
  ...
  } else if (below(lacReviewTimelinessRate, 80) && meets(lacReviewTimelinessRate, 50) && totalLacReviews > 0) {
    concerns.push(
  ...
  if (below(socialWorkerVisitRate, 50) && totalSWVisits > 0) {
    concerns.push(
  ...
  } else if (below(socialWorkerVisitRate, 80) && meets(socialWorkerVisitRate, 50) && totalSWVisits > 0) {
    concerns.push(
  ...
  if (below(therapeuticEngagementRate, 40) && totalTherapeuticRecords > 0) {
    concerns.push(
  ...
  } else if (below(therapeuticEngagementRate, 70) && meets(therapeuticEngagementRate, 40) && totalTherapeuticRecords > 0) {
    concerns.push(
  ...
  if (below(educationLiaisonRate, 60) && totalEducationLiaisons > 0) {
    concerns.push(
  ...
  } else if (below(educationLiaisonRate, 80) && meets(educationLiaisonRate, 60) && totalEducationLiaisons > 0) {
    concerns.push(
  ...
  if (below(informationSharingRate, 50) && totalInfoSharingRecords > 0) {
    concerns.push(
  ...
  } else if (below(informationSharingRate, 80) && meets(informationSharingRate, 50) && totalInfoSharingRecords > 0) {
    concerns.push(
  ...
  } else if (meets(waitingListRate, 40) && totalTherapeuticRecords > 0) {
    concerns.push(
  ...
  if (below(childSeenAloneRate, 60) && totalSWVisits > 0) {
    concerns.push(
  ...
  } else if (below(childSeenAloneRate, 80) && meets(childSeenAloneRate, 60) && totalSWVisits > 0) {
    concerns.push(
  ...
  if (below(pepUpToDateRate, 50) && totalEducationLiaisons > 0) {
    concerns.push(
  ...
  } else if (below(pepUpToDateRate, 70) && meets(pepUpToDateRate, 50) && totalEducationLiaisons > 0) {
    concerns.push(
  ...
  if (meets(lacPoorOutcomeRate, 30) && totalLacReviews > 0) {
    concerns.push(
  ...
  if (below(gdprComplianceRate, 70) && totalInfoSharingRecords > 0) {
    concerns.push(
  ...
  if (below(therapeuticInfoSharedRate, 60) && totalTherapeuticRecords > 0) {
    concerns.push(
  ...
  if (meets(swPoorQualityRate, 25) && totalSWVisits > 0) {
    concerns.push(
  ...
  if (meets(lacReviewTimelinessRate, 50) && below(lacReviewTimelinessRate, 80) && totalLacReviews > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (meets(socialWorkerVisitRate, 50) && below(socialWorkerVisitRate, 80) && totalSWVisits > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (meets(therapeuticEngagementRate, 40) && below(therapeuticEngagementRate, 70) && totalTherapeuticRecords > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (meets(educationLiaisonRate, 60) && below(educationLiaisonRate, 80) && totalEducationLiaisons > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (meets(informationSharingRate, 50) && below(informationSharingRate, 80) && totalInfoSharingRecords > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (below(lacActionCompletionRate, 70) && lacActionsSet > 0) {
    recommendations.push({
      rank: ++rank,
      recommendation:
  ...
  if (meets(pepUpToDateRate, 50) && below(pepUpToDateRate, 70) && totalEducationLiaisons > 0) {
    insights.push({
  ...
  if (below(swConsistencyRate, 70) && totalSWVisits > 0) {
    insights.push({
  ...
  if (meets(lacActionCompletionRate, 50) && below(lacActionCompletionRate, 70) && lacActionsSet > 0) {
    insights.push({
  ...
  if (below(followUpCompletionRate, 70) && followUpRequired > 0) {
    insights.push({
  ...
  if (meets(lacReviewTimelinessRate, 95) && meets(socialWorkerVisitRate, 95) && totalLacReviews > 0 && totalSWVisits > 0) {
    insights.push({
  ...
  if (meets(therapeuticEngagementRate, 90) && meets(therapeuticProgressRate, 90) && totalTherapeuticRecords > 0) {
    insights.push({
  ...
  if (meets(educationLiaisonRate, 95) && meets(pepUpToDateRate, 95) && totalEducationLiaisons > 0) {
    insights.push({
  ...
  if (meets(informationSharingRate, 95) && meets(gdprComplianceRate, 95) && totalInfoSharingRecords > 0) {
    insights.push({
  ...
  if (uniqueAgencyCount >= 5 && meets(homeInitiatedRate, 70) && totalInfoSharingRecords > 0) {
    insights.push({
  ...
  if (meets(childSeenAloneRate, 95) && meets(lacChildViewsRate, 95) && totalSWVisits > 0 && totalLacReviews > 0) {
    insights.push({
