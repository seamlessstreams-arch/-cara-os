...
export interface HolidayExperiencesResult {
  ...
  /** null when the population is empty — nothing measured, not 0%. */
  photos_documented_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  child_voice_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  anniversaries_planned_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  positive_anniversary_rate: number | null;
  strengths: string[];
  concerns: string[];
  recommendations: {
    rank: number;
...
export function computeHolidayEnrichingExperiences(
  ...
  if (total_children === 0) {
    return {
      ...
      photos_documented_rate: null,
      child_voice_rate: null,
      anniversaries_planned_rate: null,
      positive_anniversary_rate: null,
      strengths: [],
      concerns: [],
      recommendations: [],
      insights: [],
  ...
  const childChoiceRate = rate(childChoice, totalHolidays);

  const photosDocumented = holidays.filter(h => h.photos_taken).length;
  const photosRate = rate(photosDocumented, totalHolidays);
  ...
  const childVoiceRate = rate(holidayVoice + annivVoice, totalVoiceItems);

  const annivPlanned = anniversaries.filter(a => a.has_upcoming_plan).length;
  const annivPlannedRate = rate(annivPlanned, anniversaries.length);

  const positiveAnniv = anniversaries.filter(a =>
    a.child_attitude === "positive" || a.child_attitude === "neutral"
  ).length;
  const positiveAnnivRate = rate(positiveAnniv, anniversaries.length);
  ...
  if (totalHolidays === 0) {
    ...
    if (meets(childChoiceRate, 90)) score += 5;
    else if (meets(childChoiceRate, 70)) score += 2;
    else if (below(childChoiceRate, 50)) score -= 5;
  }
  ...
  if (totalHolidays === 0) {
    ...
    if (meets(photosRate, 90)) score += 6;
    else if (meets(photosRate, 70)) score += 2;
    else if (below(photosRate, 50)) score -= 5;
  }
  ...
  if (anniversaries.length === 0) {
    ...
    if (meets(annivPlannedRate, 90)) score += 5;
    else if (meets(annivPlannedRate, 70)) score += 2;
    else if (below(annivPlannedRate, 50)) score -= 5;
  }
  ...
  if (anniversaries.length === 0) {
    ...
    if (meets(positiveAnnivRate, 90)) score += 4;
    else if (meets(positiveAnnivRate, 70)) score += 1;
    else if (below(positiveAnnivRate, 50)) score -= 4;
  }
  ...
  if (meets(childChoiceRate, 90) && totalHolidays > 0) strengths.push("Children consistently choose their own holiday destinations — promoting agency and independence");
  if (meets(photosRate, 90) && totalHolidays > 0) strengths.push("Excellent photo documentation creates lasting memories for children's life story work");
  if (meets(childVoiceRate, 90) && totalVoiceItems > 0) strengths.push("Children's voices are consistently captured in holiday and anniversary planning");
  if (meets(annivPlannedRate, 90) && anniversaries.length > 0) strengths.push("All significant anniversaries have proactive plans in place");
  if (meets(positiveAnnivRate, 90) && anniversaries.length > 0) strengths.push("Children experience anniversaries positively thanks to thoughtful support");
  ...
  if (below(childChoiceRate, 50) && totalHolidays > 0) concerns.push("Children are not consistently involved in choosing holiday destinations");
  if (below(photosRate, 50) && totalHolidays > 0) concerns.push("Poor photo documentation means memories are not being preserved for children");
  if (below(childVoiceRate, 50) && totalVoiceItems > 0) concerns.push("Children's views are not being captured about their experiences and important dates");
  if (below(annivPlannedRate, 50) && anniversaries.length > 0) concerns.push("Significant anniversaries lack proactive planning — children may not feel supported");
  if (below(positiveAnnivRate, 50) && anniversaries.length > 0) concerns.push("Many children experience anniversaries negatively — review support approaches");
  ...
  if (below(childChoiceRate, 70) && totalHolidays > 0) {
  ...
  if (below(photosRate, 70) && totalHolidays > 0) {
  ...
  if (below(childVoiceRate, 70) && totalVoiceItems > 0) {
  ...
  if (below(annivPlannedRate, 70) && anniversaries.length > 0) {
  ...
  if (below(positiveAnnivRate, 70) && anniversaries.length > 0) {
  ...
  if (meets(childChoiceRate, 90) && meets(photosRate, 90) && totalHolidays > 0) {
  ...
  if (below(positiveAnnivRate, 50) && anniversaries.length > 0) {
  ...
  if (meets(annivPlannedRate, 90) && meets(positiveAnnivRate, 80) && anniversaries.length > 0) {
  ...
  if (below(childVoiceRate, 50) && totalVoiceItems > 0) {
