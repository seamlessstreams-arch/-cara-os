...
type DatedRecord = { child_id?: string | null; date?: unknown; created_at?: unknown; reported_at?: unknown };
...
export async function GET(request: NextRequest) {
  ...
  if (denied) return denied;
    ...
    const nameOf = (c: YoungPerson): string =>
    ...
    if (childId) {
      ...
      const dateOf = (r: DatedRecord): string => (r?.date ?? r?.created_at ?? r?.reported_at ?? "").toString().slice(0, 10);
      const byChild = <T extends DatedRecord>(arr: T[] | undefined, extra?: (r: T) => boolean): string[] =>
        (((arr ?? [])))
      ...
      const metrics: TrendMetricInput[] = [
        ...
        { key: "concerning_behaviour", label: "Concerning behaviour", unit: "incidents", polarity: "lower_better", description: "Behaviour-log entries recorded as concerning.", dates: byChild((behaviourLogList), (r) => r.direction === "concern") },
