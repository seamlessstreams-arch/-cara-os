...
describe("cara/health-appointments (live leg, mocked client)", () => {
  it("resolves the child from young_people — not the phantom children table", async () => {
    const res = await GET(new NextRequest("http://localhost/api/cara/health-appointments?childId=yp-live-1"));
