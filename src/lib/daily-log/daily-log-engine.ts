...
export interface DailyLogQualityResult {
  ...
  detailedObservationRate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  childMoodRate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  keyworkerInformedRate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  actionFollowedUpRate: number | null;
  score: number;
  strengths: string[];
  concerns: string[];
}
...
export interface StaffDailyLogReadinessResult {
  totalStaff: number;
  /** null when the population is empty — nothing measured, not 0%. */
  observationSkillsRate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  ...
  /** null when the population is empty — nothing measured, not 0%. */
  reflectiveWritingRate: number | null;
  score: number;
  strengths: string[];
  concerns: string[];
}
...
export function evaluateDailyLogQuality(
  ...
  if (totalRecords === 0) {
    return {
      totalRecords: 0,
      detailedObservationRate: null,
      childMoodRate: null,
      keyworkerInformedRate: null,
      actionFollowedUpRate: null,
      score: 0,
      strengths: [],
  ...
  const detailedObservationRate = rate(detailedCount, totalRecords);

  const moodCount = records.filter((r) => r.childMoodRecorded).length;
  const childMoodRate = rate(moodCount, totalRecords);

  const keyworkerCount = records.filter((r) => r.keyworkerInformed).length;
  const keyworkerInformedRate = rate(keyworkerCount, totalRecords);

  const actionCount = records.filter((r) => r.actionFollowedUp).length;
  const actionFollowedUpRate = rate(actionCount, totalRecords);
...
export function evaluateDailyLogCompliance(
  ...
  if (meets(timelyRecordingRate, 80)) {
...
export function evaluateStaffDailyLogReadiness(
  ...
  const observationSkillsRate = rate(obsCount, totalStaff);

  const recCount = staff.filter((s) => s.recordKeeping).length;
  const recordKeepingRate = rate(recCount, totalStaff);

  const commCount = staff.filter((s) => s.childCommunication).length;
  const childCommunicationRate = rate(commCount, totalStaff);

  const safCount = staff.filter((s) => s.safeguardingAwareness).length;
  const safeguardingAwarenessRate = rate(safCount, totalStaff);

  const handCount = staff.filter((s) => s.handoverPractice).length;
  const handoverPracticeRate = rate(handCount, totalStaff);

  const refCount = staff.filter((s) => s.reflectiveWriting).length;
  const reflectiveWritingRate = rate(refCount, totalStaff);
  ...
  score += ((reflectiveWritingRate ?? 0) / 100) * 2;
  score = Math.round(score * 10) / 10;
  score = Math.max(0, Math.min(25, score));
...
export function buildChildDailyLogProfiles(
  ...
  return Array.from(childMap.values()).map((child) => {
    ...
    let rate1Score = 0;
    if (meets(detailedObservationRate, 80)) rate1Score = 3;
    else if (meets(detailedObservationRate, 60)) rate1Score = 2;
    else if (meets(detailedObservationRate, 40)) rate1Score = 1;

    // rate2 (childMoodRate): same thresholds
    let rate2Score = 0;
    if (meets(childMoodRate, 80)) rate2Score = 3;
    else if (meets(childMoodRate, 60)) rate2Score = 2;
    else if (meets(childMoodRate, 40)) rate2Score = 1;
...
function generateActions(
  ...
  if (quality.totalRecords > 0 && below(quality.detailedObservationRate, 50)) {
    actions.push("HIGH: Detailed observation rate at " + quality.detailedObservationRate + "% — improve observation depth and quality");
  }

  if (quality.totalRecords > 0 && below(quality.childMoodRate, 50)) {
