...
import type { CareEventQualityRating, CareEventQualityResult } from "@/lib/engines/home-care-event-quality-intelligence-engine";
...
function useHomeCareEventQualityIntelligence() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: CareEventQualityResult }>;
    },
      ...
      <CardContent className="space-y-4">
        {d.event_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.recording_quality_rate === null ? "bg-slate-50" : d.recording_quality_rate >= 98 ? "bg-green-50" : d.recording_quality_rate >= 90 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.recording_quality_rate === null ? "text-muted-foreground" : d.recording_quality_rate >= 98 ? "text-[--cs-success]" : d.recording_quality_rate >= 90 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.recording_quality_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Quality</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.verification_rate === null ? "bg-slate-50" : d.verification_rate >= 95 ? "bg-green-50" : d.verification_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.verification_rate === null ? "text-muted-foreground" : d.verification_rate >= 95 ? "text-[--cs-success]" : d.verification_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.verification_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Verified</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.routing_completion_rate === null ? "bg-slate-50" : d.routing_completion_rate >= 95 ? "bg-green-50" : d.routing_completion_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.routing_completion_rate === null ? "text-muted-foreground" : d.routing_completion_rate >= 95 ? "text-[--cs-success]" : d.routing_completion_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.routing_completion_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Routed</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.audit_trail_rate === null ? "bg-slate-50" : d.audit_trail_rate >= 90 ? "bg-green-50" : d.audit_trail_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.audit_trail_rate === null ? "text-muted-foreground" : d.audit_trail_rate >= 90 ? "text-[--cs-success]" : d.audit_trail_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.audit_trail_rate)}</p>
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Care Event Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
