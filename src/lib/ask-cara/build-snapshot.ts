...
function buildOpsIntelligence(store: ReturnType<typeof getStore>, todayIso: string): AskCaraOpsIntelligence | undefined {
  try {
    // The key is checked against the store's own collections. It used to be a
    // bare `string`, and `rows("buildingChecks")` — a collection that
    // does not exist — silently returned [], so Cara never reported an overdue
    // building-safety check or an action arising from one.
    const rows = (k: keyof typeof store) => {
      const v = store[k];
    ...
    const checks = rows("buildingChecks");
...
export function buildAskSnapshot(store: ReturnType<typeof getStore>): AskCaraSnapshot {
  ...
  return {
    ...
    shifts: rec(store.shifts).map((sh) => ({ id: String(sh.id), staffId: String(sh.staff_id), date: day(sh.date), shiftType: s(sh.shift_type) || undefined })),
    keyWork: rec(store.keyWorkingSessions).map((k) => ({ childId: String(k.child_id), date: day(k.date ?? k.session_date) })),
    home: (() => {
      const h = store.home as unknown as Record<string, unknown> | undefined;
    ...
    contacts: rec(store.professionalNetworkContacts).map((c) => ({ childId: String(c.child_id), role: s(c.role), name: s(c.name), organisation: s(c.organisation) || undefined, phone: s(c.phone) || undefined })),
    supervisions: [
      ...
      ...rec(store.reflectiveSupervisions).map((sv) => ({ staffId: String(sv.staff_id), date: day(sv.date), nextDate: day(sv.follow_up_date) || undefined, status: "reflective" })),
    ...
    policies: rec(store.homePolicies).map((p) => ({
      id: String(p.id),
    ...
    healthAppointments: rec(store.healthRecordEntries)
      .map((h) => ({
