        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div><Label htmlFor="7b6e-agency-staff-name">Agency Staff Name *</Label><Input id="7b6e-agency-staff-name" placeholder="Full name" value={form.agency_staff_name} onChange={(e) => setForm((f) => ({ ...f, agency_staff_name: e.target.value }))} /></div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><Label htmlFor="7b6e-professionalism-1-5">Professionalism (1–5)</Label><Input id="7b6e-professionalism-1-5" type="number" min={1} max={5} value={form.professionalism_rating} onChange={(e) => setForm((f) => ({ ...f, professionalism_rating: parseInt(e.target.value) }))} /></div>
              <div><Label htmlFor="7b6e-relational-skills-1-5">Relational Skills (1–5)</Label><Input id="7b6e-relational-skills-1-5" type="number" min={1} max={5} value={form.relational_skills_rating} onChange={(e) => setForm((f) => ({ ...f, relational_skills_rating: parseInt(e.target.value) }))} /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label htmlFor="7b6e-recording-quality">Recording Quality</Label><Select value={form.recording_quality} onValueChange={(v) => setForm((f) => ({ ...f, recording_quality: v as RecordingQuality }))}><SelectTrigger id="7b6e-recording-quality"><SelectValue /></SelectTrigger><SelectContent>{(Object.entries(RECORDING_QUALITY_LABEL) as [RecordingQuality, string][]).map(([v, l]) => <SelectItem key={v} value={v}>{l}</SelectItem>)}</SelectContent></Select></div>
              <div><Label htmlFor="7b6e-verdict">Verdict</Label><Select value={form.overall_verdict} onValueChange={(v) => setForm((f) => ({ ...f, overall_verdict: v as AgencyVerdict }))}><SelectTrigger id="7b6e-verdict"><SelectValue /></SelectTrigger><SelectContent>{(Object.entries(AGENCY_VERDICT_LABEL) as [AgencyVerdict, string][]).map(([v, l]) => <SelectItem key={v} value={v}>{l}</SelectItem>)}</SelectContent></Select></div>
            </div>
            <div><Label htmlFor="7b6e-feedback-summary">Feedback Summary *</Label><Textarea id="7b6e-feedback-summary" placeholder="Key observations and decisions..." value={form.feedback_summary} onChange={(e) => setForm((f) => ({ ...f, feedback_summary: e.target.value }))} rows={3} /></div>
          </div>
          <DialogFooter>
