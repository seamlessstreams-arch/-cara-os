...
describe("staff-lone-working-service", () => {
  describe("computeStaffLoneWorkingMetrics", () => {
    ...
    it("returns empty breakdowns", () => { 
      const m = computeStaffLoneWorkingMetrics([]);
      expect(m.by_scenario).toEqual({});
      expect(m.by_risk_level).toEqual({});
      expect(m.by_check_in_frequency).toEqual({});
      expect(m.by_authorisation_level).toEqual({});
    });
    ...
    it("returns 100% boolean rates with defaults", () => { 
      const m = computeStaffLoneWorkingMetrics([makeRecord()]);
      expect(m.risk_assessed_rate).toBe(100);
      expect(m.manager_authorised_rate).toBe(100);
      expect(m.communication_plan_rate).toBe(100);
      expect(m.emergency_contacts_rate).toBe(100);
      expect(m.phone_charged_rate).toBe(100);
      expect(m.check_in_protocol_rate).toBe(100);
      expect(m.buddy_system_rate).toBe(100);
      expect(m.panic_alarm_rate).toBe(100);
      ...
      expect(m.policy_read_rate).toBe(100);
    });
  ...
  describe("identifyStaffLoneWorkingAlerts", () => {
    ...
    it("fires very_high_not_authorised", () => { 
      const a = identifyStaffLoneWorkingAlerts([makeRecord({ risk_level: "very_high", manager_authorised: false, staff_name: "Jo", lone_working_scenario: "emergency_cover" })]);
      expect(a[0].type).toBe("very_high_not_authorised");
      expect(a[0].severity).toBe("critical");
      expect(a[0].message).toContain("Jo");
      expect(a[0].message).toContain("emergency cover");
    });
    ...
    it("fires not_risk_assessed singular", () => { 
      const a = identifyStaffLoneWorkingAlerts([makeRecord({ risk_assessed: false })]);
      const f = a.find(x => x.type === "not_risk_assessed");
      expect(f).toBeDefined();
      expect(f!.severity).toBe("high");
      expect(f!.message).toContain("1 lone working arrangement has");
    });
    ...
    it("fires no_communication_plan singular", () => { 
      const a = identifyStaffLoneWorkingAlerts([makeRecord({ communication_plan: false })]);
      const f = a.find(x => x.type === "no_communication_plan");
      expect(f).toBeDefined();
      expect(f!.severity).toBe("high");
      expect(f!.message).toContain("1 lone working arrangement has");
    });
    ...
    it("fires all applicable", () => { 
      const a = identifyStaffLoneWorkingAlerts([makeRecord({ risk_level: "very_high", manager_authorised: false, risk_assessed: false, communication_plan: false, check_in_protocol_agreed: false, lone_working_policy_read: false }), makeRecord({ check_in_protocol_agreed: false, lone_working_policy_read: false })]);
      const types = a.map(x => x.type);
      expect(types).toContain("very_high_not_authorised");
      expect(types).toContain("not_risk_assessed");
      expect(types).toContain("no_communication_plan");
