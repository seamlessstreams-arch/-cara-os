...
async function fetchData(sb: NonNullable<ReturnType<typeof createServerClient>>, childId: string): Promise<ComplaintsInput> {
  ...
  const complaints: Complaint[] = (rawComplaints ?? []).map((c: any) => ({
    ...
    acknowledgedWithin24Hours: c.acknowledged_24h ?? null,
    investigatedProperly: c.investigated ?? null,
    childKeptInformed: c.child_informed ?? null,
  ...
  return {
    ...
    age,
    complaints,
    complaintsProcessExplained: complaintsConfig?.process_explained ?? null,
    childKnowsHowToComplain: complaintsConfig?.child_knows ?? null,
    advocateAvailable: complaintsConfig?.advocate_available ?? null,
    complaintsDisplayedAccessibly: complaintsConfig?.displayed_accessibly ?? null,
    independentVisitorAssigned: complaintsConfig?.iv_assigned ?? false,
    regulatoryBodyInfoProvided: complaintsConfig?.ofsted_info_provided ?? null,
    complaintsReviewedByRM: complaintsConfig?.rm_reviews ?? null,
