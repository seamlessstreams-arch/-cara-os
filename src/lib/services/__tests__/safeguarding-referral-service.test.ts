  ...
  describe("computeSafeguardingReferralMetrics", () => {
    describe("empty records", () => {
      it("returns zeros for all fields", () => {
        ...
        expect(m.consent_obtained_rate).toBeNull();;
        expect(m.information_shared_rate).toBeNull();;
        expect(m.manager_informed_rate).toBeNull();;
        expect(m.ofsted_notified_rate).toBeNull();;
        expect(m.lado_consulted_rate).toBeNull();;
        expect(m.strategy_meeting_rate).toBeNull();;
        expect(m.outcome_communicated_rate).toBeNull();;
