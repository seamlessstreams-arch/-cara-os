...
describe("computeCarePlanMetrics", () => {
  it("returns zeroes for empty data", () => {
    ...
    expect(result.plan_coverage_rate).toBeNull();; expect(result.total_objectives).toBe(0);
    expect(result.objectives_completed).toBe(0);
    expect(result.objectives_at_risk).toBe(0);
    expect(result.objective_completion_rate).toBeNull();; expect(result.reviews_this_quarter).toBe(0);
