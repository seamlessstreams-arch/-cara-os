...
export interface DigitalSafetyResult {
  ...
  digital_skill_coverage_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  parental_controls_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  rse_digital_coverage_rate: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
  exploitation_risk_assessed_rate: number | null;
  strengths: string[];
  concerns: string[];
...
export function computeDigitalLiteracyOnlineSafety(
  ...
  if (total_children === 0) {
    return {
      ...
      rse_digital_coverage_rate: null,
      exploitation_risk_assessed_rate: null,
      strengths: [],
  ...
  const digitalPlanCoverageRate = rate(uniqueChildrenWithPlans, total_children);
  ...
  const skillCompetencyRate = rate(skillsAtIntermediateOrAdvanced, digital_skills.length);
  ...
  const parentalControlsRate = rate(phonesWithControls, phone_records.length);
  ...
  const exploitationRiskAssessedRate = rate(plansWithExploitationRisk, digital_plans.length);
  ...
  const rseDigitalCoverageRate = rate(uniqueChildrenWithEngagedRse, total_children);
  ...
  const cyberbullyingPreparednessRate = rate(plansWithCyberbullyingResponse, digital_plans.length);
  ...
  const digitalSkillCoverageRate = rate(uniqueChildrenWithSkills, total_children);
  ...
  // Strengths
  if (meets(digitalPlanCoverageRate, 80)) {
    strengths.push(`${formatRate(digitalPlanCoverageRate)} of children have digital safety plans — proactive online protection embedded in care planning.`);
  }
  if (meets(skillCompetencyRate, 70)) {
    strengths.push(`${formatRate(skillCompetencyRate)} of digital skills at intermediate or advanced level — children are developing strong digital literacy.`);
  }
  if (meets(parentalControlsRate, 90) && phone_records.length > 0) {
    strengths.push("Parental controls active on the vast majority of devices — robust technical safeguards in place.");
  }
  if (meets(exploitationRiskAssessedRate, 80) && digital_plans.length > 0) {
  ...
  if (meets(rseDigitalCoverageRate, 70)) {
    strengths.push(`${formatRate(rseDigitalCoverageRate)} RSE digital safety coverage with engaged children — education programme is effective.`);
  }
  if (meets(cyberbullyingPreparednessRate, 80) && digital_plans.length > 0) {
  ...
  // Concerns
  if (below(digitalPlanCoverageRate, 80) && total_children > uniqueChildrenWithPlans) {
  ...
  if (below(skillCompetencyRate, 45) && digital_skills.length > 0) {
    concerns.push(`Only ${formatRate(skillCompetencyRate)} of digital skills at intermediate level or above — children may lack the competency to stay safe online.`);
  }
  if (below(parentalControlsRate, 70) && phone_records.length > 0) {
    concerns.push(`Parental controls active on only ${formatRate(parentalControlsRate)} of devices — children may be exposed to unfiltered online content.`);
  }
  if (below(exploitationRiskAssessedRate, 50) && digital_plans.length > 0) {
    concerns.push(`Only ${formatRate(exploitationRiskAssessedRate)} of digital plans include exploitation risk assessments — online exploitation risks may be unidentified.`);
  }
  if (below(rseDigitalCoverageRate, 40)) {
    concerns.push(`RSE digital safety coverage at ${formatRate(rseDigitalCoverageRate)} — too few children receiving engaged digital safety education.`);
  }
  if (below(cyberbullyingPreparednessRate, 50) && digital_plans.length > 0) {
    concerns.push(`Only ${formatRate(cyberbullyingPreparednessRate)} of plans include cyberbullying response — the home may be unprepared for online harassment incidents.`);
  }

  // Recommendations
  if (below(digitalPlanCoverageRate, 80)) {
    recommendations.push({
      ...
      urgency: below(digitalPlanCoverageRate, 30) ? "immediate" : "soon",
  ...
  if (below(skillCompetencyRate, 45) && digital_skills.length > 0) {
    recommendations.push({
      ...
      urgency: below(skillCompetencyRate, 25) ? "immediate" : "planned",
      regulatory_ref: "CHR 2015 Reg 12",
    });
  }
  if (below(parentalControlsRate, 70) && phone_records.length > 0) {
    recommendations.push({
      ...
      urgency: below(parentalControlsRate, 50) ? "immediate" : "soon",
      regulatory_ref: "CHR 2015 Reg 13",
    });
  }
  if (below(exploitationRiskAssessedRate, 80) && digital_plans.length > 0) {
    recommendations.push({
      rank: ++rank,
      ...
      urgency: below(exploitationRiskAssessedRate, 30) ? "immediate" : "soon",
      regulatory_ref: "CHR 2015 Reg 13",
    });
  }
  if (below(rseDigitalCoverageRate, 70)) {
    recommendations.push({
      rank: ++rank,
      ...
      urgency: below(rseDigitalCoverageRate, 20) ? "immediate" : "planned",
      regulatory_ref: "CHR 2015 Reg 12",
    });
  }
  if (below(cyberbullyingPreparednessRate, 80) && digital_plans.length > 0) {
    recommendations.push({
      rank: ++rank,
      ...
      urgency: below(cyberbullyingPreparednessRate, 30) ? "immediate" : "soon",
      regulatory_ref: "CHR 2015 Reg 13",
    });
  }
  ...
  // Insights
  if (
    meets(digitalPlanCoverageRate, 80) &&
    meets(skillCompetencyRate, 70) &&
    meets(parentalControlsRate, 90) &&
    meets(rseDigitalCoverageRate, 70) &&
    meets(exploitationRiskAssessedRate, 80) &&
    meets(cyberbullyingPreparednessRate, 80)
  ) {
    insights.push({
  ...
  if (below(parentalControlsRate, 50) && phone_records.length > 0) {
    insights.push({
      text: `Parental controls active on only ${formatRate(parentalControlsRate)} of devices. Without technical safeguards, children are reliant solely on their own digital literacy to avoid harmful content — this is a significant safeguarding gap.`,
      severity: "critical",
    });
  }
  if (below(exploitationRiskAssessedRate, 30) && digital_plans.length > 0) {
    insights.push({
      text: `Only ${formatRate(exploitationRiskAssessedRate)} of digital plans include exploitation risk assessments. Online exploitation (grooming, CSE, radicalisation) is a primary Ofsted concern — this gap could trigger a regulatory finding.`,
      severity: "critical",
    });
  }
  if (below(rseDigitalCoverageRate, 20)) {
    insights.push({
      text: `RSE digital safety coverage is critically low at ${formatRate(rseDigitalCoverageRate)}. Children are not receiving education about online risks — this leaves them vulnerable and may breach Reg 12 expectations for health and wellbeing.`,
      severity: "critical",
    });
  }
  if (below(skillCompetencyRate, 25) && digital_skills.length > 0) {
    insights.push({
      text: `Only ${formatRate(skillCompetencyRate)} of digital skills are at intermediate or above. Most children lack the competency to navigate online risks independently — direct work and structured digital literacy programmes are urgently needed.`,
      severity: "warning",
    });
  }
  if (below(cyberbullyingPreparednessRate, 30) && digital_plans.length > 0) {
    insights.push({
      text: `Cyberbullying preparedness is critically low at ${formatRate(cyberbullyingPreparednessRate)}. Without response plans, staff lack clear guidance when online harassment occurs — response times and outcomes will suffer.`,
      severity: "warning",
    });
  }
  ...
  if (digital_rating === "outstanding") {
    headline = `Outstanding digital literacy and online safety — ${uniqueChildrenWithPlans} children with plans, ${formatRate(skillCompetencyRate)} skills at intermediate+, robust safeguards in place.`;
  } else if (digital_rating === "good") {
    headline = `Good digital safety posture — ${formatRate(digitalPlanCoverageRate)} plan coverage, ${formatRate(skillCompetencyRate)} skill competency.${concerns.length > 0 ? ` ${concerns.length} area${concerns.length > 1 ? "s" : ""} for improvement.` : ""}`;
