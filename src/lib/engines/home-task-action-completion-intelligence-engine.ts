...
export interface TaskActionCompletionResult {
  ...
  /** null when the population is empty — nothing measured, not 0%. */
  incident_follow_through: number | null;
  /** null when the population is empty — nothing measured, not 0%. */
...
export function computeTaskActionCompletion(
  ...
  const completionRate = rate(completedTasks.length, tasks.length);
  ...
  const onTimeRate = rate(onTimeTasks.length, completedWithDue.length);
  ...
  const incidentFollowThrough = rate(incidentsWithTask.length, incidents.length);
  ...
  const urgentCompletionRate = rate(completedUrgent.length, urgentTasks.length);
  ...
  const signOffRate = rate(signedOff.length, requiresSignOff.length);

  // Escalated tasks
  const escalatedTasks = tasks.filter(t => t.escalated);
  const escalationRate = rate(escalatedTasks.length, tasks.length);
  ...
  // 1. Completion rate
  if (meets(completionRate, 95)) score += 6;
  else if (meets(completionRate, 80)) score += 3;
  else if (below(completionRate, 50)) score -= 8;
  else if (below(completionRate, 65)) score -= 5;

  // 2. Overdue management (overdue as pct of total tasks)
  const overdueRate = rate(overdueCount, tasks.length);
  if (tasks.length === 0) {
    score += 0; // no tasks means no overdue — neutral
  } else if (overdueRate === 0) score += 5;
  else if ((overdueRate !== null && overdueRate <= 5)) score += 2;
  else if (above(overdueRate, 20)) score -= 5;

  // 3. On-time rate
  if (completedWithDue.length === 0) {
    score -= 1; // no due-dated completed tasks — slight concern
  } else {
    if (meets(onTimeRate, 95)) score += 5;
  ...
  if (incidents.length === 0) {
    ...
    if (meets(incidentFollowThrough, 95)) score += 5;
    else if (meets(incidentFollowThrough, 80)) score += 2;
    else if (below(incidentFollowThrough, 60)) score -= 4;
  }
  ...
  if (urgentTasks.length === 0) {
    ...
    if (meets(urgentCompletionRate, 95)) score += 4;
    else if (meets(urgentCompletionRate, 80)) score += 2;
    else if (below(urgentCompletionRate, 60)) score -= 4;
  }
  ...
  if (requiresSignOff.length === 0) {
    ...
    if (meets(signOffRate, 95)) score += 5;
    else if (meets(signOffRate, 80)) score += 2;
    else if (below(signOffRate, 60)) score -= 4;
  }
  ...
  if (below(completionRate, 65) && tasks.length > 0) {
  ...
  if (above(overdueRate, 20) && tasks.length > 0) {
  ...
  if (below(onTimeRate, 60) && completedWithDue.length > 0) {
  ...
  if (below(incidentFollowThrough, 60) && incidents.length > 0) {
  ...
  if (below(urgentCompletionRate, 60) && urgentTasks.length > 0) {
  ...
  if (below(signOffRate, 60) && requiresSignOff.length > 0) {
  ...
  if (above(escalationRate, 25) && tasks.length > 0) {
  ...
  if (meets(completionRate, 95) && meets(onTimeRate, 95) && overdueRate === 0 && tasks.length > 0) {
  ...
  if (meets(incidentFollowThrough, 95) && incidents.length > 0 && meets(completionRate, 80)) {
  ...
  if (below(completionRate, 50) && tasks.length > 0) {
