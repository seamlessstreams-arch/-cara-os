...
function makeRecord(overrides?: Partial<PhysicalActivityTrackingRecord>): PhysicalActivityTrackingRecord {
  return {
    ...
    participation_level: "willing",
    fitness_assessment: "average",
...
describe("physical-activity-tracking-service", () => {
  describe("computePhysicalActivityMetrics", () => {
    ...
    it("returns empty breakdowns", () => { 
      ...
      expect(m.by_fitness_assessment).toEqual({});
    ...
    it("returns 100% boolean rates with defaults", () => { 
      const m = computePhysicalActivityMetrics([makeRecord()]);
      ...
      expect(m.inclusive_rate).toBe(100);
  ...
  describe("identifyPhysicalActivityAlerts", () => {
    ...
    it("fires refused_no_health_check", () => { 
      const a = identifyPhysicalActivityAlerts([makeRecord({ participation_level: "refused", health_needs_considered: false, child_name: "Jo" })]);
      expect(a[0].type).toBe("refused_no_health_check");
    ...
    it("fires no_child_choice singular", () => { 
      const a = identifyPhysicalActivityAlerts([makeRecord({ child_choice_offered: false })]);
    ...
    it("fires risk_not_assessed singular", () => { 
      const a = identifyPhysicalActivityAlerts([makeRecord({ risk_assessed: false })]);
      const f = a.find(x => x.type === "risk_not_assessed");
      expect(f).toBeDefined();
      expect(f!.severity).toBe("high");
      expect(f!.message).toContain("1 activity has");
    });
    ...
    it("fires all applicable", () => { 
      const a = identifyPhysicalActivityAlerts([makeRecord({ participation_level: "refused", health_needs_considered: false, child_choice_offered: false, risk_assessed: false, achievement_celebrated: false, inclusive_activity: false }), makeRecord({ child_choice_offered: false, risk_assessed: false, achievement_celebrated: false, inclusive_activity: false })]);
      const types = a.map(x => x.type);
      expect(types).toContain("refused_no_health_check");
      expect(types).toContain("no_child_choice");
      expect(types).toContain("risk_not_assessed");
