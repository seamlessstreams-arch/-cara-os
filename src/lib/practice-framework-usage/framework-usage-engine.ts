// ══════════════════════════════════════════════════════════════════════════════
// CARA — PRACTICE FRAMEWORK USAGE INTELLIGENCE — pure engine
//
...
//
// All deterministic. No LLM calls. Pure — the store is passed IN, so this is
// fully unit-testable and never touches NextResponse or getStore() itself.
// ══════════════════════════════════════════════════════════════════════════════
...
export type FrameworkId = keyof typeof KB_FRAMEWORKS;
export const FRAMEWORK_IDS = Object.keys(KB_FRAMEWORKS) as FrameworkId[];

// ── Source → framework mappings ───────────────────────────────────────────────

export const WA_ISSUE_TO_FW: Record<string, FrameworkId> = {
  tone:                  "model_pace",
...
export const DOMAIN_TO_FW: Record<string, FrameworkId> = {
...
export type Signal = "active" | "emerging" | "dormant";
export type Trend  = "increasing" | "stable" | "declining";

export function signal(total: number): Signal {
  if (total >= 10) return "active";
  if (total >= 3)  return "emerging";
  return "dormant";
}

export function trend(recentCount: number, priorCount: number): Trend {
...
export const SUPERVISION_PROMPTS: Record<FrameworkId, Record<Signal, string>> = {
  model_pace: {
...
// ── Result types — the exact `data` payload the route returns ────────────────
...
export interface FrameworkUsageSummary {
  ...
  mostActiveFramework: { id: FrameworkId; title: string } | null;
  needsAttentionFramework: { id: FrameworkId; title: string } | null;
  topPractitioner: TopEngager | null;
  sourceBreakdown: Counts["sources"];
}
