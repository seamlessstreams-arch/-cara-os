...
describe("parental-contact-arrangement-service", () => {
  describe("computeParentalContactMetrics", () => {
    ...
    it("court_compliance_rate 0 when no court_ordered rows exist", () => { const m = computeParentalContactMetrics([makeRow({ court_order_status: "agreed_informally" })]); expect(m.court_compliance_rate).toBeNull(); });
