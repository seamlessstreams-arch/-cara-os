...
import type { CctvGovernanceRating, CctvGovernanceResult } from "@/lib/engines/home-cctv-surveillance-governance-intelligence-engine";
...
function useHomeCctvSurveillanceGovernanceIntelligence() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: CctvGovernanceResult }>;
    },
      ...
      <CardContent className="space-y-4">
        {d.cctv_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", meets(d.policy_compliance_rate, 90) ? "bg-green-50" : meets(d.policy_compliance_rate, 70) ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", meets(d.policy_compliance_rate, 90) ? "text-[--cs-success]" : meets(d.policy_compliance_rate, 70) ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.policy_compliance_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Policy</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", meets(d.privacy_impact_rate, 90) ? "bg-green-50" : meets(d.privacy_impact_rate, 70) ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", meets(d.privacy_impact_rate, 90) ? "text-[--cs-success]" : meets(d.privacy_impact_rate, 70) ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.privacy_impact_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Privacy</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", meets(d.retention_compliance_rate, 90) ? "bg-green-50" : meets(d.retention_compliance_rate, 70) ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", meets(d.retention_compliance_rate, 90) ? "text-[--cs-success]" : meets(d.retention_compliance_rate, 70) ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.retention_compliance_rate)}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", meets(d.data_protection_rate, 90) ? "bg-green-50" : meets(d.data_protection_rate, 70) ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", meets(d.data_protection_rate, 90) ? "text-[--cs-success]" : meets(d.data_protection_rate, 70) ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.data_protection_rate)}</p>
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara CCTV & Surveillance Governance Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
