...
import type { NotificationResponsivenessRating, NotificationResponsivenessResult } from "@/lib/engines/home-notification-responsiveness-intelligence-engine";

function useHomeNotificationResponsivenessIntelligence() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: NotificationResponsivenessResult }>;
    },
      ...
      <CardContent className="space-y-4">
        {d.responsiveness_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.read_rate === null ? "bg-slate-50" : d.read_rate >= 90 ? "bg-green-50" : d.read_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.read_rate === null ? "text-muted-foreground" : d.read_rate >= 90 ? "text-[--cs-success]" : d.read_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.read_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Read</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.urgent_read_rate === null ? "bg-slate-50" : d.urgent_read_rate >= 100 ? "bg-green-50" : d.urgent_read_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.urgent_read_rate === null ? "text-muted-foreground" : d.urgent_read_rate >= 100 ? "text-[--cs-success]" : d.urgent_read_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.urgent_read_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Urgent</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.average_response_hours === null ? "bg-slate-50" : d.average_response_hours <= 2 ? "bg-green-50" : d.average_response_hours <= 6 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.average_response_hours === null ? "text-muted-foreground" : d.average_response_hours <= 2 ? "text-[--cs-success]" : d.average_response_hours <= 6 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.average_response_hours}h</p>
