...
import type { InductionOnboardingRating, StaffInductionOnboardingResult } from "@/lib/engines/home-staff-induction-onboarding-intelligence-engine";
...
function useHomeStaffInductionOnboardingIntelligence() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: StaffInductionOnboardingResult }>;
    },
      ...
      <CardContent className="space-y-4">
        {d.induction_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.completion_rate === null ? "bg-slate-50" : d.completion_rate >= 90 ? "bg-green-50" : d.completion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.completion_rate === null ? "text-muted-foreground" : d.completion_rate >= 90 ? "text-[--cs-success]" : d.completion_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.completion_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Complete</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.safeguarding_coverage_rate === null ? "bg-slate-50" : d.safeguarding_coverage_rate >= 100 ? "bg-green-50" : d.safeguarding_coverage_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.safeguarding_coverage_rate === null ? "text-muted-foreground" : d.safeguarding_coverage_rate >= 100 ? "text-[--cs-success]" : d.safeguarding_coverage_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.safeguarding_coverage_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Safegrd</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.shadowing_competency_rate === null ? "bg-slate-50" : d.shadowing_competency_rate >= 90 ? "bg-green-50" : d.shadowing_competency_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.shadowing_competency_rate === null ? "text-muted-foreground" : d.shadowing_competency_rate >= 90 ? "text-[--cs-success]" : d.shadowing_competency_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.shadowing_competency_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Shadow</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.handbook_acknowledgement_rate === null ? "bg-slate-50" : d.handbook_acknowledgement_rate >= 90 ? "bg-green-50" : d.handbook_acknowledgement_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.handbook_acknowledgement_rate === null ? "text-muted-foreground" : d.handbook_acknowledgement_rate >= 90 ? "text-[--cs-success]" : d.handbook_acknowledgement_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.handbook_acknowledgement_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Handbook</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.agency_induction_completion_rate === null ? "bg-slate-50" : d.agency_induction_completion_rate >= 100 ? "bg-green-50" : d.agency_induction_completion_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.agency_induction_completion_rate === null ? "text-muted-foreground" : d.agency_induction_completion_rate >= 100 ? "text-[--cs-success]" : d.agency_induction_completion_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.agency_induction_completion_rate)}</p>
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Induction Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
