      ...
      <Dialog open={showNew} onOpenChange={setShowNew}>
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleSavePlan} className="space-y-3 py-2">
            <div><Label htmlFor="156e-young-person">Young Person *</Label><Select value={phForm.child_id} onValueChange={(v) => setPH("child_id", v)}><SelectTrigger id="156e-young-person" className="mt-1"><SelectValue placeholder="Select young person…" /></SelectTrigger><SelectContent>{YOUNG_PEOPLE.filter((y) => y.status === "current").map((y) => (<SelectItem key={y.id} value={y.id}>{y.preferred_name ?? y.first_name}</SelectItem>))}</SelectContent></Select></div>
            <div><Label htmlFor="156e-reviewed-by">Reviewed By</Label><Select value={phForm.reviewed_by} onValueChange={(v) => setPH("reviewed_by", v)}><SelectTrigger id="156e-reviewed-by" className="mt-1"><SelectValue placeholder="Select staff…" /></SelectTrigger><SelectContent><SelectItem value="">TBC</SelectItem>{STAFF.filter((s) => s.employment_status === "active").map((s) => (<SelectItem key={s.id} value={s.id}>{s.full_name}</SelectItem>))}</SelectContent></Select></div>
            <div><Label htmlFor="156e-known-triggers-one-per-line">Known Triggers (one per line)</Label><Textarea id="156e-known-triggers-one-per-line" className="mt-1" rows={3} placeholder="e.g. Loud noises, crowded spaces…" value={phForm.triggers} onChange={(e) => setPH("triggers", e.target.value)} /></div>
            <div><Label htmlFor="156e-notes">Notes</Label><Textarea id="156e-notes" className="mt-1" rows={2} value={phForm.notes} onChange={(e) => setPH("notes", e.target.value)} /></div>
