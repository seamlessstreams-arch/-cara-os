...
import type { DigitalInclusionRating, TechnologyDigitalInclusionResult } from "@/lib/engines/home-technology-digital-inclusion-intelligence-engine";
...
export function HomeTechnologyDigitalInclusionIntelligenceCard() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: TechnologyDigitalInclusionResult }>;
    },
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-indigo-600 border-2")} aria-label="Cara Technology &amp; Digital Inclusion Intelligence">
      ...
      <CardContent className="space-y-4">
        {d.digital_inclusion_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", meets(d.internet_safety_rate, 90) ? "bg-green-50" : meets(d.internet_safety_rate, 70) ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", meets(d.internet_safety_rate, 90) ? "text-[--cs-success]" : meets(d.internet_safety_rate, 70) ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.internet_safety_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Safety</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", meets(d.technology_learning_rate, 90) ? "bg-green-50" : meets(d.technology_learning_rate, 70) ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", meets(d.technology_learning_rate, 90) ? "text-[--cs-success]" : meets(d.technology_learning_rate, 70) ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.technology_learning_rate)}</p>
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Technology & Digital Inclusion Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
