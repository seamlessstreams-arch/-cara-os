...
function useHomeStaffLoneWorkingSafetyIntelligence() {
  return useQuery<StaffLoneWorkingSafetyResponse>({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: StaffLoneWorkingResult }>;
    },
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-red-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            {d.lone_working_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{formatRate(d.lone_working_score)}</span>}
          </CardTitle>
        </div>
      ...
      <CardContent className="space-y-4">
        {d.lone_working_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", (d.risk_assessment_rate ?? 0) >= 90 ? "bg-green-50" : (d.risk_assessment_rate ?? 0) >= 70 ? "bg-amber-50" : "bg-red-50")}><p className={cn("text-sm font-bold tabular-nums", (d.risk_assessment_rate ?? 0) >= 90 ? "text-[--cs-success]" : (d.risk_assessment_rate ?? 0) >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.risk_assessment_rate)}</p><p className="text-[9px] text-muted-foreground">Risk Asst.</p></div>
            <div className={cn("text-center rounded-lg p-1.5", (d.check_in_compliance_rate ?? 0) >= 90 ? "bg-green-50" : (d.check_in_compliance_rate ?? 0) >= 70 ? "bg-amber-50" : "bg-red-50")}><p className={cn("text-sm font-bold tabular-nums", (d.check_in_compliance_rate ?? 0) >= 90 ? "text-[--cs-success]" : (d.check_in_compliance_rate ?? 0) >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.check_in_compliance_rate)}</p><p className="text-[9px] text-muted-foreground">Check-in</p></div>
            <div className={cn("text-center rounded-lg p-1.5", (d.safety_protocol_rate ?? 0) >= 90 ? "bg-green-50" : (d.safety_protocol_rate ?? 0) >= 70 ? "bg-amber-50" : "bg-red-50")}><p className={cn("text-sm font-bold tabular-nums", (d.safety_protocol_rate ?? 0) >= 90 ? "text-[--cs-success]" : (d.safety_protocol_rate ?? 0) >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.safety_protocol_rate)}</p><p className="text-[9px] text-muted-foreground">Protocol</p></div>
            <div className={cn("text-center rounded-lg p-1.5", (d.communication_device_rate ?? 0) >= 90 ? "bg-green-50" : (d.communication_device_rate ?? 0) >= 70 ? "bg-amber-50" : "bg-red-50")}><p className={cn("text-sm font-bold tabular-nums", (d.communication_device_rate ?? 0) >= 90 ? "text-[--cs-success]" : (d.communication_device_rate ?? 0) >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.communication_device_rate)}</p><p className="text-[9px] text-muted-foreground">Comms</p></div>
            <div className={cn("text-center rounded-lg p-1.5", (d.incident_reporting_rate ?? 0) >= 90 ? "bg-green-50" : (d.incident_reporting_rate ?? 0) >= 70 ? "bg-amber-50" : "bg-red-50")}><p className={cn("text-sm font-bold tabular-nums", (d.incident_reporting_rate ?? 0) >= 90 ? "text-[--cs-success]" : (d.incident_reporting_rate ?? 0) >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.incident_reporting_rate)}</p><p className="text-[9px] text-muted-foreground">Incidents</p></div>
            <div className={cn("text-center rounded-lg p-1.5", (d.staff_confidence_rate ?? 0) >= 90 ? "bg-green-50" : (d.staff_confidence_rate ?? 0) >= 70 ? "bg-amber-50" : "bg-red-50")}><p className={cn("text-sm font-bold tabular-nums", (d.staff_confidence_rate ?? 0) >= 90 ? "text-[--cs-success]" : (d.staff_confidence_rate ?? 0) >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.staff_confidence_rate)}</p><p className="text-[9px] text-muted-foreground">Confid.</p></div>
          </div>
        )}
