...
import type { KeyWorkerRating, KeyWorkerRelationshipQualityResult } from "@/lib/engines/home-key-worker-relationship-quality-intelligence-engine";
...
export function HomeKeyWorkerRelationshipQualityIntelligenceCard() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: KeyWorkerRelationshipQualityResult }>;
    },
      ...
      <CardContent className="space-y-4">
        {d.key_worker_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", meets(d.allocation_coverage_rate, 90) ? "bg-green-50" : meets(d.allocation_coverage_rate, 70) ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", meets(d.allocation_coverage_rate, 90) ? "text-[--cs-success]" : meets(d.allocation_coverage_rate, 70) ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.allocation_coverage_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Allocated</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", meets(d.relationship_quality_rate, 90) ? "bg-green-50" : meets(d.relationship_quality_rate, 70) ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", meets(d.relationship_quality_rate, 90) ? "text-[--cs-success]" : meets(d.relationship_quality_rate, 70) ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.relationship_quality_rate)}</p>
              <p className="text-[9px] text-muted-foreground">Quality</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", meets(d.session_regularity_rate, 90) ? "bg-green-50" : meets(d.session_regularity_rate, 70) ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", meets(d.session_regularity_rate, 90) ? "text-[--cs-success]" : meets(d.session_regularity_rate, 70) ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.session_regularity_rate)}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", meets(d.continuity_rate, 90) ? "bg-green-50" : meets(d.continuity_rate, 70) ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", meets(d.continuity_rate, 90) ? "text-[--cs-success]" : meets(d.continuity_rate, 70) ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.continuity_rate)}</p>
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Key Worker Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
