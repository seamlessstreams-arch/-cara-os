...
function skillRepairCycle(snap: AskCaraSnapshot, child: AskCaraChild | null): AskCaraAnswer {
  ...
  if (child) {
    ...
    const lines = [`${name}'s repair cycles are ${pc.completionRate === null ? "not measurable — no incidents on record" : `${pc.completionRate}% complete`}${pc.noRepair ? ` — ${pc.noRepair} incident${pc.noRepair === 1 ? " has" : "s have"} no repair recorded at all` : ""}${pc.missingStep && pc.missingStep !== "None" ? `. The step most often missing: ${pc.missingStep.toLowerCase()}` : ""}.`, "", `Repair is where the relationship learns it can survive rupture — the conversations matter more than the paperwork, and the paperwork shows whether they happened.`];
    return answer({ intent: "repair_cycle", answered: true, text: lines.join("\n"), sources: [...(pc.completionRate === null ? [] : [{ label: `Repair completion for ${name} (%)`, count: pc.completionRate }]), { label: "No repair recorded", count: pc.noRepair }], suggestions: sug([`How are ${name}'s relationships?`, "Which restraints have no debrief?"]), disclaimer: DISC });
  }
  const lines = [`Across the home, repair cycles are ${rc.overallCompletionRate === null ? "not measurable — no incidents on record" : `${rc.overallCompletionRate}% complete`} over ${rc.totalIncidents} incident${rc.totalIncidents === 1 ? "" : "s"} — ${rc.incidentsWithChildPerspective} captured the child's perspective${rc.avgDebriefTurnaroundDays !== null ? `, and debriefs happen on average ${rc.avgDebriefTurnaroundDays} day${rc.avgDebriefTurnaroundDays === 1 ? "" : "s"} after the incident` : ""}.`];
  ...
  return answer({ intent: "repair_cycle", answered: true, text: lines.join("\n"), sources: [...(rc.overallCompletionRate === null ? [] : [{ label: "Repair completion (%)", count: rc.overallCompletionRate }]), { label: "Incidents", count: rc.totalIncidents }], suggestions: sug(["Which restraints have no debrief?", "How are the children's relationships?", "What needs my attention?"]), disclaimer: DISC });
}
...
function skillWeeklySummary(snap: AskCaraSnapshot, child: AskCaraChild | null, period: "week" | "month" = "week"): AskCaraAnswer {
  ...
  lines.push(`**Emotional wellbeing:** ${w.emotionalWellbeing}`);
  if (w.qualityStandardsEvidence.length) {
    lines.push("", "**Quality Standards evidence:**");
    for (const e of w.qualityStandardsEvidence) lines.push(`- ${e.label}: ${e.evidence}`);
  }
  if (w.fiveOutcomesEvidence.length) {
    lines.push("", "**Five Outcomes evidence:**");
    for (const e of w.fiveOutcomesEvidence) lines.push(`- ${e.label}: ${e.evidence}`);
  }
