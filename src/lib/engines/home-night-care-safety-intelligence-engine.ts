...
export function computeHomeNightCareSafety(
  ...
  if (
    ...
    return {
      ...
      anxiety_support: { total_records: 0, child_coverage: null, avg_strategies: null, severe_crisis_with_referral_rate: null, child_voice_rate: null, child_preferences_rate: null },
      bedtime_routines: { total_routines: 0, child_coverage: null, avg_effectiveness: null, child_agreed_rate: null, avg_steps: null, overdue_reviews: 0 },
      wake_up_routines: { total_routines: 0, child_coverage: null, avg_effectiveness: null, child_agreed_rate: null, avg_steps: null, overdue_reviews: 0 },
  ...
  const ncTempOkRate = rate(ncTempOk, nc30.length);
  ...
  const ncConcernFollowUpRate = rate(ncConcernFollowUps, ncConcernRaised);
  ...
  const hoCompletionRate = rate(hoComplete, night_staff_handovers.length);
  ...
  const nasCoverage = rate(nasChildIds.size, total_children);
  ...
  const brCoverage = rate(brChildIds.size, total_children);
  ...
  const brAgreedRate = rate(brAgreed, bedtime_routines.length);
  ...
  const wuCoverage = rate(wuChildIds.size, total_children);
  ...
  const wuAgreedRate = rate(wuAgreed, wake_up_routines.length);
  ...
  const sleepingRate = rate(ncSleeping, nc30.length);
  const settledRate = rate(ncSettled, nc30.length);
  const distressedRate = rate(ncDistressed, nc30.length);
  const notInRoomRate = rate(ncNotInRoom, nc30.length);
  ...
  // ── Mod 1: Night Check Completeness & Frequency (±5) ────────────────
  {
    ...
    if (nc30.length > 0) {
      ...
      // Room temp ok rate
      if (meets(ncTempOkRate, 95)) m += 1;
      else if (below(ncTempOkRate, 70)) m -= 1;

      // Concern follow-up rate
      if (ncConcernRaised > 0) {
        if (meets(ncConcernFollowUpRate, 80)) m += 1;
        else if (below(ncConcernFollowUpRate, 40)) m -= 1;
      } else {
        m += 1; // No concerns raised is fine
      }

      // Scheduled vs additional balance: having some additional checks is good practice
      const additionalRate = rate(ncAdditional, nc30.length);
      if (meets(additionalRate, 10) && additionalRate! <= 40) m += 1;
  ...
  // ── Mod 2: Handover Quality (±4) ───────────────────────────────────
  {
    ...
    if (night_staff_handovers.length > 0) {
      ...
      if (meets(hoCompletionRate, 90)) m += 2;
      else if (meets(hoCompletionRate, 70)) m += 1;
      else m -= 1;
  ...
  // ── Mod 3: Night Anxiety Support Coverage (±3) ──────────────────────
  {
    ...
    if (night_anxiety_support_records.length > 0) {
      // Child coverage
      if (meets(nasCoverage, 80)) m += 1;
      else if (below(nasCoverage, 40)) m -= 1;
      ...
      if (nasSevereCrisis.length > 0) {
        if (meets(nasSCReferralRate, 80)) m += 1;
        else if (below(nasSCReferralRate, 40)) m -= 1;
      } else {
  ...
  // ── Mod 4: Bedtime Routine Coverage & Quality (±4) ──────────────────
  {
    ...
    if (bedtime_routines.length > 0) {
      // Child coverage
      if (meets(brCoverage, 80)) m += 1;
      else if (below(brCoverage, 40)) m -= 1;
      ...
      // Child agreed rate
      if (meets(brAgreedRate, 80)) m += 1;
      else if (below(brAgreedRate, 40)) m -= 1;
  ...
  // ── Mod 5: Wake-Up Routine Coverage & Quality (±3) ──────────────────
  {
    ...
    if (wake_up_routines.length > 0) {
      // Child coverage
      if (meets(wuCoverage, 80)) m += 1;
      else if (below(wuCoverage, 40)) m -= 1;
      ...
      if (meets(wuAgreedRate, 80) && meets(wuAvgSteps, 4)) m += 1;
      else if (below(wuAgreedRate, 40)) m -= 1;
    } else {
  ...
  // ── Mod 6: Sleep Quality & Concern Management (±3) ──────────────────
  {
    ...
    if (nc30.length > 0) {
      ...
      const goodRate = rate(ncSleeping + ncSettled, nc30.length);
      if (meets(goodRate, 85)) m += 2;
      else if (meets(goodRate, 65)) m += 1;
      else if (below(goodRate, 40)) m -= 1;

      // Distressed / not_in_room as negatives
      const badRate = rate(ncDistressed + ncNotInRoom, nc30.length);
      if (above(badRate, 20)) m -= 2;
      else if (above(badRate, 10)) m -= 1;
  ...
  if (nc30.length > 0 && meets(ncTempOkRate, 95)) {
  ...
  if (nc30.length > 0 && below(ncTempOkRate, 70)) {
  ...
  // Handovers
  if (night_staff_handovers.length > 0 && meets(hoCompletionRate, 90)) {
    strengths.push(`Strong handover practice — ${hoCompletionRate}% of morning handovers completed.`);
  }
  if (night_staff_handovers.length > 0 && below(hoCompletionRate, 70)) {
  ...
  // Anxiety support
  if (night_anxiety_support_records.length > 0 && meets(nasCoverage, 80) && meets(nasAvgStrategies, 6)) {
  ...
  if (nasSevereCrisis.length > 0 && below(nasSCReferralRate, 40)) {
  ...
  // Bedtime routines
  if (bedtime_routines.length > 0 && meets(brCoverage, 80) && meets(brAvgEff, 4)) {
  ...
  // Wake-up routines
  if (wake_up_routines.length > 0 && meets(wuCoverage, 80) && meets(wuAvgEff, 4)) {
  ...
  if (night_staff_handovers.length >= 5 && below(hoCompletionRate, 50)) {
  ...
  if (nc30.length > 0 && meets(sleepingRate, 80) && distressedRate === 0 && notInRoomRate === 0) {
  ...
  if (bedtime_routines.length > 0 && meets(brAgreedRate, 90) && wake_up_routines.length > 0 && meets(wuAgreedRate, 90)) {
