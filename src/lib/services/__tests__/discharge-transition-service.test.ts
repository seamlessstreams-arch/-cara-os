...
describe("computeDischargeMetrics", () => {
  it("returns zeroed metrics for empty reviews array", () => {
    ...
    expect(m.social_worker_involved_rate).toBeNull();;
    expect(m.family_consulted_rate).toBeNull();;
    expect(m.education_plan_rate).toBeNull();;
    expect(m.health_transferred_rate).toBeNull();;
    expect(m.life_story_rate).toBeNull();;
    expect(m.goodbye_event_rate).toBeNull();;
