...
import type { CaraContentRating, CaraContentQualityResult } from "@/lib/engines/home-cara-content-quality-intelligence-engine";
...
function useHomeCaraContentQualityIntelligence() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: CaraContentQualityResult }>;
    },
      ...
      <CardContent className="space-y-4">
        {d.content_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.approval_rate === null ? "bg-slate-50" : d.approval_rate >= 90 ? "bg-green-50" : d.approval_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.approval_rate === null ? "text-muted-foreground" : d.approval_rate >= 90 ? "text-[--cs-success]" : d.approval_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.approval_rate)}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.framework_usage_rate === null ? "bg-slate-50" : d.framework_usage_rate >= 80 ? "bg-green-50" : d.framework_usage_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.framework_usage_rate === null ? "text-muted-foreground" : d.framework_usage_rate >= 80 ? "text-[--cs-success]" : d.framework_usage_rate >= 50 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{formatRate(d.framework_usage_rate)}</p>
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Content Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
