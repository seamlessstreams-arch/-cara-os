...
describe("cara/education-engagement (live leg, mocked client)", () => {
  it("resolves the child from young_people — not the phantom children table", async () => {
    const res = await GET(new NextRequest("http://localhost/api/cara/education-engagement?childId=yp-live-1"));
