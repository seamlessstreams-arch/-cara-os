...
import type { DampMouldRating, DampMouldManagementResult } from "@/lib/engines/home-damp-mould-management-intelligence-engine";
...
export function HomeDampMouldManagementIntelligenceCard() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ data: DampMouldManagementResult }>;
    },
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-amber-600 border-2")}>
      ...
      <CardContent className="space-y-4">
        ...
        {d.insights?.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Damp & Mould Management Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
